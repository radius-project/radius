...
The ref those actions resolve at is worth knowing before you rely on either property. Generated workflows emit `uses: radius-project/ai-extensions/.github/extension/actions/<name>@{{RADIUS_REF}}`, and the generator substitutes `RADIUS_SOURCE_REF`. A **released** build stamps that value at bundle time with the exact `ai-extensions` commit it was built from, and the release validator rejects any build whose ref is not a full 40-character SHA. The `main` default in [`packages/core/src/workflows/deploy.ts`](https://github.com/radius-project/ai-extensions/blob/main/packages/core/src/workflows/deploy.ts) is a development fallback for unstamped local builds, not what users get.
