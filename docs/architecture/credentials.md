...
An optional `Radius.Core/terraformSettings.properties.backend` selects existing S3 or Azure Blob state storage. The Terraform executor uses the same typed UCP credential providers described above, calling `Fetch(ctx, "aws" or "azurecloud", "default")` on **every deploy/update/delete execution**. It does so independently of the recipe's required providers: a Kubernetes-only recipe can use S3 state and still needs registered AWS credentials. Backend configuration contains only the state location and locking options, never credentials. See [Terraform state backends](terraform-bicep-settings.md#terraform-state-backends) for examples, storage permissions, immutability, and state retention.

- **AWS AccessKey:** passes registered access key ID and secret access key via `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`.
- **AWS IRSA:** passes the registered role via `AWS_ROLE_ARN` and the existing projection `/var/run/secrets/eks.amazonaws.com/serviceaccount/token` via `AWS_WEB_IDENTITY_TOKEN_FILE`. Terraform performs the STS exchange.
- **Azure ServicePrincipal:** passes registered client ID, tenant ID, and client secret via `ARM_CLIENT_ID`, `ARM_TENANT_ID`, and `ARM_CLIENT_SECRET`.
- **Azure WorkloadIdentity:** passes registered client ID and tenant ID, sets `ARM_USE_OIDC=true`, and uses `ARM_OIDC_TOKEN_FILE_PATH=/var/run/secrets/azure/tokens/azure-identity-token`. This reuses the supported WorkloadIdentity registration, not a new VM managed identity registration.

Azure backend rendering always enables `use_azuread_auth`, so Terraform uses Entra Blob data-plane access rather than listing account keys. Azure CLI and MSI fallback are disabled. No secret is placed in backend JSON or backend CLI arguments, and the executor does not mutate global environment variables or log credential values.

The executor merges inherited environment, settings environment variables, resolved legacy environment secrets, and generated `TF_CLI_CONFIG_FILE`, then applies the registered backend credential **last**. Selected-cloud authentication fields are authoritative: stale AWS static keys, session tokens, profile and web-identity selectors are removed before setting the chosen mode; shared AWS credential/config files and instance metadata are disabled for this execution. Stale Azure client-ID file (`ARM_CLIENT_ID_FILE_PATH`), secret/certificate, storage key/SAS, MSI, and OIDC selectors are cleared or explicitly disabled before setting the selected mode. Inherited or settings-supplied credential file paths cannot override either registered Azure mode; only WorkloadIdentity's registered-mode OIDC token path is retained. The other cloud and unrelated environment variables are preserved. Missing, malformed, or unsupported selected-cloud registrations are errors, not provider-style fallback to host credentials.

**Provider interaction:** Terraform backends and providers share a process environment. Existing explicit provider configuration and Radius's provider configuration precedence remain unchanged. Providers that rely on environment authentication in the selected cloud will see the backend's registered identity, and AWS providers cannot rely on inherited shared-profile files in that execution. To intentionally use a different provider identity, supply complete explicit provider authentication supported by that provider; do not use conflicting global authentication environment variables. The backend credential remains the registered default and is never inferred from provider configuration. This feature does not change existing provider credential fallback when no cloud backend is selected.

**AWS endpoint overrides:** With managed S3 state, the executor rejects nonempty global, S3, and STS endpoint overrides in the merged environment before launching Terraform. This prevents redirected state traffic and IRSA token exchanges. Overrides are rejected, not silently removed, to avoid redirecting provider operations intended for custom endpoints to real AWS. See [credentials and permissions](terraform-bicep-settings.md#credentials-and-permissions) for the exact variables and policy; Kubernetes and Azure backends retain their existing endpoint behavior.
