...
4. **[Release controller](https://github.com/radius-project/radius/actions/workflows/release-controller.yaml)** (`release-controller.yaml`): Triggered when a generated release pull request is squash-merged to `main`, when its generated metadata backport is rebase-merged to `release/<channel>`, or by an approved default-branch dispatch from Approve Release or Resume Release. This workflow:
   ...
   A release branch that was cut before this controller existed does not carry these workflow files, so merging its metadata backport may not start the controller. Every branch cut after the controller landed inherits them from `main`. For those older channels, merge the metadata backport as usual and then start the same validated run with [Approve Release](#resuming-a-release), passing the version and the backport merge commit on `release/X.Y`.
