...
docker run \
  ...
  -e RAD_LOCAL_DEVELOPMENT=true \ # Required: Enables local development mode
  -e KUBECONFIG=/root/.kube/config \ # Optional: If not provided, the default kubeconfig path will be used.
  -p 5017:8080 \
  ghcr.io/radius-project/deployment-engine:latest
```

- **RADIUSBACKENDURL:** Specifies the backend URL for the Universal Control Plane (UCP).
- **RAD_LOCAL_DEVELOPMENT:** Ensures the Deployment Engine operates in local development mode.
- **KUBECONFIG (Optional):** Path to the Kubernetes configuration file inside the container. If not set, the Deployment Engine will attempt to use the default kubeconfig file path.

> 💡 Note: The RAD_LOCAL_DEVELOPMENT environment variable is required to enable local development mode. The KUBECONFIG variable is optional and only needed if you want to specify a custom kubeconfig file path.
...
5. Make sure that the `RAD_LOCAL_DEVELOPMENT` environment variable in launch.json is set to true. This will make sure that Deployment Engine knows that a local development is in progress.
6. The context used by the active Radius workspace should also be the active Kubernetes context. Otherwise, you will get an error from the Deployment Engine.
