...
The canonical versions, release sources, platform assets, and SHA-256 checksums for downloaded CLI tools live in [`build/tools.yaml`](../../../../build/tools.yaml). Install the Updatecli version in [`.updatecli-version`](../../../../.updatecli-version), then use `make check-tools` to preview updates or `make update-tools` to apply them locally. Native Updatecli pipelines replace the custom Go updater and regenerate the committed [`build/tools.generated.mk`](../../../../build/tools.generated.mk) include used by `make install-<tool>`. After manual manifest edits, run `make generate-tools` to synchronize the include and version consumers without network access; ordinary builds do not invoke Updatecli. The PoC has no release-age filtering, but does not downgrade versions and keeps Bicep compatibility-pinned. See the [Updatecli reference](../../../../.updatecli/README.md) for setup, Windows executable selection, supported metadata, and the manual-only publishing workflow.
