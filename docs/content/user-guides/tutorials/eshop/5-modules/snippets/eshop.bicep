...
module catalog 'catalog.bicep' = {
  ...
  params: {
    ...
    OCHESTRATOR_TYPE: OCHESTRATOR_TYPE
    servicebus: infra.outputs.serviceBus
    sqlCatalog: infra.outputs.sqlCatalog
  }
}
...
module identity 'identity.bicep' = {
  ...
  params: {
    ...
    APPLICATION_INSIGHTS_KEY: APPLICATION_INSIGHTS_KEY
    OCHESTRATOR_TYPE: OCHESTRATOR_TYPE
    basketHttp: basket.outputs.basketHttp
    orderingHttp: ordering.outputs.orderingHttp
    sqlIdentity: infra.outputs.sqlIdentity
    webhooksclientHttp: webhooks.outputs.webhooksclientHttp
    webhooksHttp: webhooks.outputs.webhooksHttp
    webmvcHttp: webmvc.outputs.webmvcHttp
    webshoppingaggHttp: webshoppingagg.outputs.webshoppingaggHttp
    webspaHttp: webspa.outputs.webspaHttp
  }
}

module ordering 'ordering.bicep' = {
  name: 'ordering'
  params: {
    app: eshop
    APPLICATION_INSIGHTS_KEY: APPLICATION_INSIGHTS_KEY
    basketHttp: basket.outputs.basketHttp
    catalogHttp: catalog.outputs.catalogHttp
    identityHttp: identity.outputs.identityHttp
    OCHESTRATOR_TYPE: OCHESTRATOR_TYPE
    servicebus: {
    }
    sqlOrdering: {
...
module webhooks 'webhooks.bicep' = {
  name: 
  params: {
    app: {
    }
...
module webshoppingapigw 'webshoppingapigw.bicep' = {
  name: 
  params: {
    app: {
    }
...
module webspa 'webspa.bicep' = {
  ...
  params: {
    ...
    webshoppingapigwHttp: {
    }
  }
}

module webmvc 'webmvc.bicep' = {
  name: 
  params: {
    app: {
    }
...
//APP
resource eshopold 'radius.dev/Application@v1alpha3' = {
  name: 'eshopold'
