...
An Azure resource that does not bind to a [Radius component type](https://radapp.dev/resources/) can be defined outside the application and added as a connection in the Radius application. To identify an azure resource, connection `kind` field should be set to `azure` and RBAC can be configured by specifying role names in the `role` field.
