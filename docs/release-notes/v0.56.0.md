...
### Default recipe pack setup during rad init and environment operations

Radius now automatically sets up a default Recipe Pack during `rad init` and Environment operations. Recipe Packs are a feature coming soon with an updated Environment which simplify defining Recipes across multiple Environments.

### Removed premature deprecation warnings for Applications.Core resources

Previously, Applications.Core resources displayed premature deprecation warnings. These have been removed to reduce noise for users who are not yet migrating.
...
### Azure provider scope fix

Fixed Azure provider scope construction for the v20250801preview API version.
