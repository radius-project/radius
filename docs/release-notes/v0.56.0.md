...
### Fixed dashboard 401 Unauthorized after Backstage upgrade

The Radius dashboard was returning `401 Unauthorized` on all pages due to a Backstage dependency upgrade that silently disables guest auth when `NODE_ENV=production`. The Helm chart now sets `NODE_ENV=development` on the dashboard container to restore guest auth, configurable via `dashboard.nodeEnv` in `values.yaml`.
