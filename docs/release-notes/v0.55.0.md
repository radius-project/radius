...
### Secrets management: sensitive property annotation and encryption

Radius v0.55.0 introduces built-in secrets management. You can now mark resource properties as sensitive using the new `x-radius-sensitive` TypeSpec/OpenAPI annotation. Radius encrypts these values at rest, redacts them from GET and LIST API responses, and supports key rotation so you can rotate encryption keys without losing existing secret values.

See [#10999](https://github.com/radius-project/radius/pull/10999), [#11007](https://github.com/radius-project/radius/pull/11007), [#11047](https://github.com/radius-project/radius/pull/11047), [#11098](https://github.com/radius-project/radius/pull/11098), [#11114](https://github.com/radius-project/radius/pull/11114), [#11157](https://github.com/radius-project/radius/pull/11157), [#11195](https://github.com/radius-project/radius/pull/11195) for details.

### Deploy by recipe digest

`rad deploy` now supports pinning recipe deployments by OCI digest, giving you reproducible deployments that are immune to tag mutation. Pass the digest instead of a tag when specifying a recipe reference to deploy an exact, immutable recipe version. See [#11057](https://github.com/radius-project/radius/pull/11057) for details.

### `rad deploy` environment improvements

`rad deploy` now supports deploying environment resources directly (#10926) and works with both Radius core environments and application core environments (#10923). The `rad env show --preview` command also shows provider details (#10925).

### Radius.Compute and Radius.Security resource types registered by default

`Radius.Compute` and `Radius.Security` resource types are now registered by default when Radius is installed, making these resource namespaces available without any manual registration step. See [#11113](https://github.com/radius-project/radius/pull/11113) for details.

### Recipe pack: support for HTTP registries

Recipe packs can now fetch recipes from an HTTP registry in addition to OCI registries, enabling more flexible recipe distribution patterns. See [#10975](https://github.com/radius-project/radius/pull/10975) for details.

### Deprecation warning for `Applications.*` namespace with the 2023-10-01-preview API version

Radius now emits a deprecation warning when resources use the `Applications.*` namespace with the `2023-10-01-preview` API version. Please migrate to the `Radius.*` namespace. See [#11153](https://github.com/radius-project/radius/pull/11153) for details.

### CLI fixes: JSON output and rollback timestamps

- `rad app graph --output json` now returns valid JSON ([#11207](https://github.com/radius-project/radius/pull/11207))
- `rad version --output json` now returns valid JSON ([#11019](https://github.com/radius-project/radius/pull/11019))
- `rad rollback kubernetes --list-revisions` now shows correct revision timestamps ([#10955](https://github.com/radius-project/radius/pull/10955))
