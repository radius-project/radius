...
Previously, recipes in Radius required specific input and output params or vars. Often, this requires users to wrap an existing Infrastructure as Code (IaC) module with a Radius-specific recipe. Direct IaC module support removes this requirement and allows users to map Resource Type properties to recipe inputs and outputs in the Environment definition.
