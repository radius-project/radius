...
# Installs the Dapr CLI (the 'dapr' binary) into a user-owned directory (no sudo)
...
# job PATH so later steps can run dapr.
#
# The Dapr CLI is published as a per-platform tarball on dapr/cli's GitHub
...
#   * empty DAPR_VERSION         -> the latest published release
#   * missing checksum for platform  -> read from the release's own
#                                       '<asset>.sha256' file
#
# Usage: install-dapr.sh [install_dir]
#
# Environment (all optional):
#   DAPR_VERSION                Release tag, e.g. v1.15.1. Empty selects latest.
#   DAPR_CHECKSUM_<OS>_<ARCH>   SHA-256 of the tarball for that platform (e.g.
#                                   DAPR_CHECKSUM_LINUX_AMD64). Empty fetches
#                                   it from the release's '<asset>.sha256' file.
#   DAPR_INSTALL_DIR            Install directory. Default: $HOME/.local/bin.
#   GITHUB_TOKEN                    If set, authenticates GitHub requests (higher
#                                   rate limits; required for private repositories).

readonly REPO="dapr/cli"
readonly RELEASES_URL="https://github.com/${REPO}/releases"

log() { echo "[install-dapr] $*" >&2; }
fail() {
    echo "[install-dapr] ERROR: $*" >&2
    exit 1
}
...
gh_curl() {
    local headers=(-H "User-Agent: dapr-cli-installer")
...
resolve_latest_version() {
    ...
    effective_url="$(gh_curl -fsSLI -o /dev/null -w '%{url_effective}' "${RELEASES_URL}/latest")" \
        || fail "could not resolve the latest Dapr CLI version"
    printf '%s\n' "${effective_url##*/tag/}"
}

# Print the SHA-256 of an asset, read from the release's own published
# '<asset>.sha256' file. dapr/cli publishes one per release artifact; each
# contains a single '<sha256>  <asset>' line.
checksum_from_release() {
    local version="$1" asset="$2"
    gh_curl -fsSL "${RELEASES_URL}/download/${version}/${asset}.sha256" -o "${WORKDIR}/${asset}.sha256" \
        || fail "could not download ${asset}.sha256 for ${version}"
    awk 'NR == 1 { print $1 }' "${WORKDIR}/${asset}.sha256"
}
...
main() {
    ...
    install_dir="${1:-${DAPR_INSTALL_DIR:-}}"
    ...
    # latest release, and accept a bare number (1.15.1) as well as a tag (v1.15.1).
    version="${DAPR_VERSION:-}"
    version="${version//[[:space:]]/}"
    if [ -z "$version" ]; then
        log "resolving latest Dapr CLI version..."
        version="$(resolve_latest_version)"
    elif [ "${version#[0-9]}" != "$version" ]; then
        version="v${version}"
    fi
    [ -n "$version" ] || fail "could not determine the Dapr CLI version to install"

    if command -v dapr >/dev/null 2>&1 && dapr --version 2>/dev/null | grep -q "${version#v}"; then
        log "Dapr CLI ${version} already installed: $(command -v dapr)"
        return 0
    fi
    ...
    # read it from the release's own published '<asset>.sha256' file.
    case "$platform" in
        linux_amd64) checksum="${DAPR_CHECKSUM_LINUX_AMD64:-}" ;;
        linux_arm64) checksum="${DAPR_CHECKSUM_LINUX_ARM64:-}" ;;
        darwin_amd64) checksum="${DAPR_CHECKSUM_DARWIN_AMD64:-}" ;;
        darwin_arm64) checksum="${DAPR_CHECKSUM_DARWIN_ARM64:-}" ;;
        *) checksum="" ;;
    esac
    if [ -z "$checksum" ]; then
    ...
    [ -f "${WORKDIR}/dapr" ] || fail "expected 'dapr' binary not found in ${asset}"
    chmod 0755 "${WORKDIR}/dapr"

    mkdir -p "$install_dir"
    mv "${WORKDIR}/dapr" "${install_dir}/dapr"
    "${install_dir}/dapr" --version >/dev/null 2>&1 \
        || fail "installed dapr failed to run (${install_dir}/dapr)"
    log "installed Dapr CLI ${version} to ${install_dir}/dapr"

    # Make dapr available to later GitHub Actions steps.
