...
- **FR-018**: The control plane exposes `GET /planes/radius/{plane}/providers/System.Resources/resourceProviders/{namespace}/resourceTypes/{type}/icons/{hash}` returning bytes with `Content-Type: image/svg+xml`, `Cache-Control: public, max-age=31536000, immutable`, and a strong `ETag` derived from the hash. A hash that doesn't match the stored bytes returns 404. Auth matches the resource-type metadata endpoints.
