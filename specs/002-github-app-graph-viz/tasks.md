...
- [X] T014 [P] [US3] Create reusable GitHub Actions workflow in .github/workflows/__build-app-graph.yml that builds `rad` from the requested Radius source ref, runs `rad graph build`, and commits `{source-branch}/app.json` to the `radius-graph` orphan branch on push events that modify app.bicep
