...
## 6. Execution plan (PRs)

We slice the work by **topic cluster**, not by workstream: each PR consolidates, reformats, and verifies *one* cluster end-to-end, so a reviewer sees the whole story for a single area and no change crosses a PR boundary. The [§3 workstreams](#3-workstreams) (WS1–WS7) describe the *kinds* of work; the label on each task below traces back to them.

Smaller is better — target XS–S PRs. **PRs 1–7 are independent** (disjoint file sets) and can land in any order or concurrently. **PRs 8–9 land last** because the indexes reference the final paths. Each PR runs the [§5 deterministic checks](#5-verification) on its own changed files before merge.

### PR 1 — Entry-point cleanup · XS · no deps

- [ ] Fold `how-to.md` into `CONTRIBUTING.md`; delete `how-to.md`; fix inbound links. (WS1)

### PR 2 — Prerequisites / dev environment · S · no deps

- [ ] Consolidate prerequisites: make `contributing-code-prerequisites/README.md` canonical; shrink the first-commit prereq/dev-tools steps to links; remove the "keep in sync" notes. (WS1)
- [ ] Decide Codespaces: restore the commented-out section with working instructions, or delete it and state why. (WS2)
- [ ] Reformat to the five-section format; verify every command/path/`make` target against current code. (WS3/WS4)

### PR 3 — Building + CLI · S · no deps

- [ ] Dedupe building and CLI: first-commit building/CLI steps link to `contributing-code-building/` and `contributing-code-cli/`. (WS1)
- [ ] Fix the dead `running-rad-cli.md` link in `first-commit-03-working-on-cli/index.md`. (WS2)
- [ ] Reformat to the five-section format; verify commands/paths/flags. (WS3/WS4)

### PR 4 — Running locally / debugging · S · no deps

- [ ] Delete the deprecated `running-controlplane-locally.md`; repoint inbound links (incl. `contributing-code-control-plane/README.md`) to `radius-os-processes-debugging.md`. (WS1)
- [ ] Reformat the authoritative doc to the five-section format; verify against current code. (WS3/WS4)

### PR 5 — Testing · M · no deps

- [ ] Resolve the testing overlap: define the unit/functional/local-iteration audience split and cross-link; remove duplicated setup prose. (WS1)
- [ ] Resolve the "under construction" integration-test section in `contributing-code-tests/README.md`. (WS2)
- [ ] Reformat `contributing-code-tests/README.md` into the authoritative test-matrix overview (+ per-tier pages); verify every test command. (WS3/WS4)

### PR 6 — Schema changes · S · no deps

- [ ] Resolve the `configSettings.md` "under construction" legacy-env-vars marker. (WS2)
- [ ] Reformat and verify the TypeSpec → Swagger → Go pipeline against current `make` targets. (WS3/WS4)

### PR 7 — Process / reference docs · S · no deps

- [ ] Add a **Purpose** paragraph + verified links to the code-review, issues/triage, releases, organization, and writing docs (keep their reference/process shape). (WS3)
- [ ] Reformat `contributing-pull-requests/README.md` to the five-section format; verify links. (WS3/WS4)

### PR 8 — Docs index · S · depends on PR 1–7

- [ ] Add the docs index to `docs/contributing/README.md`: one grouped list of every contributing doc, replacing the three competing link lists. (WS5)

### PR 9 — Capability index + path map · M · depends on PR 8

- [ ] Resolve the 1.5–1.8 primary-doc question; write any short primary docs needed. (WS6)
- [ ] Add the capability index to `docs/contributing/README.md`. (WS6)
- [ ] Seed the code↔doc path map in `contributing-agent-assets.md`. (WS7)
- [ ] Re-evaluate the Phase 5/6 backlog in light of the audit; record the trimmed backlog in the PR description. (gate)

### Final acceptance (not a PR)

- [ ] Run the [§5 deterministic checks](#5-verification) across the whole tree and the five canonical prompt questions.

### Batching for parallel work

| Batch | PRs | When | Why batched this way |
|---|---|---|---|
| **A** | PR 1 – PR 7 | Start together | Disjoint file sets, no cross-dependencies — open and review all seven in parallel. |
| **B** | PR 8 | After all of Batch A merges | The docs index links to the final paths produced by PR 1 – PR 7. |
| **C** | PR 9 | After PR 8 merges | The capability index and code↔doc path map reference the docs index from PR 8. |

Within Batch A, merge order doesn't matter (no shared files). Batches B and C are single-PR gates, so the critical path is **Batch A (parallel) → PR 8 → PR 9**.
