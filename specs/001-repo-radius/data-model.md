# Data Model: Git Workspace Mode

**Feature**: 001-repo-radius | **Date**: 2026-02-02

This document defines the key entities, their fields, relationships, and validation rules for the Git workspace mode feature.
...
```go
// pkg/cli/git/config/environment.go
...
// GitWorkspace represents the built-in Git workspace for Git workspace mode
...
workspaces:
  # Built-in Git workspace for Git workspace mode (local/decentralized)
  git:
    connection:
      kind: git
...
> **Terminology Note**: In Control Plane Radius, recipes are managed via "Recipe Pack" resources with full metadata. In Git workspace mode, we use the simpler term "recipes" since they're just YAML files containing recipe mappings. The `RECIPES` environment variable replaces what would be `RECIPE_PACKS` in Control Plane terminology.
