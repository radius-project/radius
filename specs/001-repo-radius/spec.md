...
- Q: What specific features should make Repo Radius well-suited for GitHub Actions? → A: Exit codes and structured output (JSON/YAML) to enable workflow conditionals and artifact parsing
- Q: What file format should be used to store deployed resource details? → A: YAML (human-readable, easy to diff in Git, widely supported, consistent with other Radius configuration files)
- Q: Does Radius need a state store for AWS resources to maintain idempotency? → A: No state store needed; AWS deployments use Terraform only (Terraform manages its own state)
...
- **FR-005**: System MUST return meaningful exit codes (0 for success, non-zero for specific failure categories) to enable GitHub Actions workflow conditionals
- **FR-006**: System MUST support structured output format (JSON/YAML) for machine parsing in CI/CD pipelines
...
- **FR-037**: System MUST store deployment details as YAML files in `.radius/deploy/`
...
- **Deployment Record**: Structured details captured after `rad deploy` completes, including Environment, resource IDs, and full resource properties. Stored as YAML files in `.radius/deploy/` for human readability and Git diffing.
...
- AWS deployments use Terraform exclusively; Terraform manages its own state, so no additional Radius state store is required for AWS idempotency
- Bicep is supported for Azure deployments only (Azure Resource Manager provides native idempotency)
...
- **FE-007**: Bicep support for AWS deployments (requires state management solution)
