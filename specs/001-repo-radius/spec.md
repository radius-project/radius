...
> **Terminology Note**: "Repo Radius" is the internal project codename for this feature. In user-facing documentation, CLI output, and code, use the formal mode names defined below.
...
As a developer, I want to generate ready-to-deploy deployment artifacts from my application model so that I can understand and audit the infrastructure changes before deployment.
...
6. **Given** a deployment step uses `recipeKind: bicep`, **When** generating artifacts for that step, **Then** the system:
   ...
   - For Azure deployments: executes `az deployment group what-if` and stores the output in `bicep-whatif.txt`
   - For Kubernetes deployments: see **Open Issue OI-001** (what-if not available for Bicep Kubernetes extension)
   - Generates a `bicep-context.log` file capturing execution context for auditability and troubleshooting:
     - Bicep CLI version
     - Azure CLI version (if deploying to Azure)
     - Target scope (resource group, subscription, etc.)
...
14. **Given** `rad plan` completes successfully, **When** the plan is generated, **Then** the system displays a summary and next steps:
    ...
    📋 Summary:
       ...
       • Artifacts saved to: .radius/plan/myapp/production/
...
18. **Given** resource details are captured, **When** writing to storage, **Then** the system stores deployment details as JSON files in `.radius/deploy/<app>/<env>/deployment-<commit-short>.json` (JSON preserves native format from each platform's API)
...
3. **Given** the migration preview is displayed, **When** I confirm the migration, **Then** the system creates corresponding resources in the Control Plane:
   ...
   - Recipe files from `.radius/config/`
...
4. **Given** the migration completes successfully, **When** I view the results, **Then** the system displays:
   ...
   💡 Next steps:
      ...
      3. Deploy via Control Plane: rad deploy .radius/model/app.bicep
...
- **What happens when the recipe file specified in `.env` does not exist?**
  - System MUST validate all recipe file paths from `.env` (via `RECIPES` variable)
  - If any file doesn't exist, system MUST display error: "Recipe file not found at <path>. Please check your .env configuration."
  - System MUST exit with code 2
...
- **What happens when multiple environments specify different deployment tools (Terraform vs Bicep)?**
  - System MUST respect the `RECIPES` specified in each `.env.<ENVIRONMENT_NAME>` file
  - Different environments MAY use different recipe files that specify different deployment tools
...
- **FR-026**: When the deployment tool is Terraform, system MUST execute `terraform plan` and store the output in `.radius/plan/<app>/<env>/<step>/terraform-plan.txt`
- **FR-027**: When the deployment tool is Bicep targeting Azure, system MUST execute `az deployment group what-if` and store the output in `.radius/plan/<app>/<env>/<step>/bicep-whatif.txt`. For Kubernetes deployments, see Open Issue OI-001.
- **FR-028**: System MUST support `--environment <name>` flag to specify which environment configuration to use
...
- **FR-045**: System MUST accept a commit hash or tag as a positional argument to specify which commit to deploy from
- **FR-046**: System MUST automatically detect and use `GITHUB_SHA` environment variable when running in GitHub Actions if no commit argument is provided
- **FR-047**: System MUST capture structured details about deployed resources after successful deployment
- **FR-048**: System MUST record the Environment used for each deployment
- **FR-049**: System MUST record cloud platform resource IDs for each deployed resource
- **FR-050**: System MUST record the full set of properties for each deployed resource as returned by the cloud platform
- **FR-051**: System MUST store deployment details as JSON files in `.radius/deploy/<app>/<env>/deployment-<commit-short>.json`
- **FR-052**: System MUST support `--environment` (`-e`) flag to specify which environment configuration to use
- **FR-053**: System MUST support `--application` (`-a`) flag to specify which application to deploy
...
- **Environment**: A deployment target configuration specifying cloud provider details, recipe files reference, and deployment tool settings. Stored as `.env` (default) or `.env.<ENVIRONMENT_NAME>` files. Configuration includes:
  - AWS: `AWS_ACCOUNT_ID`, `AWS_REGION`
  - Azure: `AZURE_SUBSCRIPTION_ID`, `AZURE_RESOURCE_GROUP`
  - Kubernetes: `KUBERNETES_CONTEXT`, `KUBERNETES_NAMESPACE`
  - Required: `RECIPES` (comma-separated list of recipe YAML paths)
  ...
  **Example: AWS + EKS (`.env.production`)**
  ...
  # Required: Recipe files (comma-separated)
  RECIPES=.radius/config/recipes.yaml
  ...
  **Example: Azure + AKS (`.env.staging`)**
  ...
  # Required: Recipe files (comma-separated, multiple allowed)
...
- **Recipes**: A YAML file (default: `recipes.yaml`) containing an array of recipe entries that map abstract resource types to deployment artifacts. Each entry contains:
  ...
  Stored in `.radius/config/`. Referenced via `RECIPES` variable in `.env` files (required).
...
### Bicep IntelliSense for Custom Resource Types (FE-011)
