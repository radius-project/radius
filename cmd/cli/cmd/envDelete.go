...
func deleteEnv(cmd *cobra.Command, args []string) error {
	...
	if ok {
		...
		if !yes {
			confirmed, err := prompt.Confirm(fmt.Sprintf("Resource group %s and all radius created resources in %s will be deleted. Continue deleting? [y/n]?", az.ControlPlaneResourceGroup, az.ResourceGroup))
			if err != nil {
				return err
			}
