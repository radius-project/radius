...
func DaprStateStoreBinding(envParams map[string]string) BindingStatus {
	...
	if stateStoreName == "" {
		log.Fatal("STATESTORENAME is required")
		return BindingStatus{false, "STATESTORENAME is required"}
	}
	client, err := dapr.NewClientWithPort(os.Getenv("DAPR_HTTP_PORT"))
	if err != nil {
	...
	if err := client.SaveState(ctx, stateStoreName, "key", []byte("value")); err != nil {
		log.Fatal("failed to save to the Dapr state store - ", stateStoreName, " error - ", err.Error())
