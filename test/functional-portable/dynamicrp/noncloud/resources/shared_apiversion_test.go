...
func Test_DynamicRP_SharedAPIVersionDeletion(t *testing.T) {
	...
	// Using dedicated test resource types to avoid interfering with other tests
	...
	test := rp.NewRPTest(t, "shared-apiversion-test", []rp.TestStep{
		{
			...
			Executor: step.NewFuncExecutor(func(ctx context.Context, t *testing.T, options test.TestOptions) {
				...
				for _, resourceType := range append(resourceTypesToPreserve, resourceTypesToDelete) {
					...
					// Check that the shared API version exists
					properties, ok := resourceTypeData["properties"].(map[string]interface{})
					require.True(t, ok, "Properties should exist in resource type")

					apiVersions, ok := properties["apiVersions"].(map[string]interface{})
					require.True(t, ok, "API versions should exist in resource type properties")
				...
				properties, ok := providerData["properties"].(map[string]interface{})
				require.True(t, ok, "Properties should exist in resource provider")

				resourceTypes, ok := properties["resourceTypes"].(map[string]interface{})
				require.True(t, ok, "Resource types should exist in resource provider properties")
				...
				for _, resourceType := range append(resourceTypesToPreserve, resourceTypesToDelete) {
					...
					require.True(t, ok, "API versions should exist for resource type %s", resourceTypeName)
		...
		},
		{
			...
			Executor: step.NewFuncExecutor(func(ctx context.Context, t *testing.T, options test.TestOptions) {
				...
				for _, resourceType := range resourceTypesToPreserve {
					...
					properties, ok := resourceTypeData["properties"].(map[string]interface{})
					require.True(t, ok, "Properties should exist in preserved resource type")

					apiVersions, ok := properties["apiVersions"].(map[string]interface{})
					require.True(t, ok, "API versions should exist in preserved resource type properties")
				...
				properties, ok := providerData["properties"].(map[string]interface{})
				require.True(t, ok, "Properties should exist in resource provider after deletion")
				
				resourceTypes, ok := properties["resourceTypes"].(map[string]interface{})
				require.True(t, ok, "Resource types should exist in resource provider properties after deletion")
				...
				for _, resourceType := range resourceTypesToPreserve {
					...
					apiVersions, ok := resourceTypeEntry["apiVersions"].(map[string]interface{})
					require.True(t, ok, "API versions should exist for preserved resource type %s", resourceTypeName)
