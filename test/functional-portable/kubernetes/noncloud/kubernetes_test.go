...
func Test_TutorialApplication_KubernetesManifests(t *testing.T) {
	...
	t.Run("Check Deployment status", func(t *testing.T) {
		...
		deployment, err = waitForDeploymentReady(ctx, t, types.NamespacedName{Name: "demo", Namespace: namespace}, opts.K8sClient, deployment.ResourceVersion)
		require.NoError(t, err)
...
func waitForDeploymentReady(ctx context.Context, t *testing.T, name types.NamespacedName, client *kubernetes.Clientset, initialVersion string) (*appsv1.Deployment, error) {
	// Based on https://gist.github.com/PrasadG193/52faed6499d2ec739f9630b9d044ffdc
	watcher, err := watchtools.NewRetryWatcherWithContext(ctx, initialVersion, cache.NewFilteredListWatchFromClient(client.AppsV1().RESTClient(), "deployments", name.Namespace, func(options *metav1.ListOptions) {
