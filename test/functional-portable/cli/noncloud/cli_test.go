	...
	test := rp.NewRPTest(t, name, []rp.TestStep{
		{
			...
			K8sObjects: &validation.K8sObjectSet{
				Namespaces: map[string][]validation.K8sObject{
					"default-group-delete-test-app": {
						validation.NewK8sPodForResource(appName, strings.ReplaceAll(containerA, "-", "")),
						validation.NewK8sPodForResource(appName, strings.ReplaceAll(containerB, "-", "")),
					},
				},
			},
...
func verifyGroupDeleteWithResources(ctx context.Context, t *testing.T, test rp.RPTest) {
	...
	// Test 2: Delete empty resource group
