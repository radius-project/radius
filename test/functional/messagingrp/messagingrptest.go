...
type TestStep struct {
	...
	MessagingRPResources                   *validation.MessagingRPResourceSet
	K8sOutputResources                     []unstructured.Unstructured
	K8sObjects                             *validation.K8sObjectSet
	AWSResources                           *validation.AWSResourceSet
	PostStepVerify                         func(ctx context.Context, t *testing.T, ct MessagingRPTest)
	SkipKubernetesOutputResourceValidation bool
	SkipObjectValidation                   bool
	SkipResourceDeletion                   bool
}

type MessagingRPTest struct {
	Options          MessagingRPTestOptions
	Name             string
	Description      string
	InitialResources []unstructured.Unstructured
	Steps            []TestStep
	PostDeleteVerify func(ctx context.Context, t *testing.T, ct MessagingRPTest)
...
func NewMessagingRPTest(t *testing.T, name string, steps []TestStep, initialResources ...unstructured.Unstructured) MessagingRPTest {
	return MessagingRPTest{
		Options:          NewMessagingRPTestOptions(t),
		Name:             name,
		Description:      name,
		Steps:            steps,
...
// K8sSecretResource creates the secret resource for Initial Resource in NewMessagingRPTest().
...
func (ct MessagingRPTest) CreateInitialResources(ctx context.Context) error {
...
func (ct MessagingRPTest) CleanUpExtensionResources(resources []unstructured.Unstructured) {
...
func (ct MessagingRPTest) CheckRequiredFeatures(ctx context.Context, t *testing.T) {
...
func (ct MessagingRPTest) Test(t *testing.T) {
	...
	for i, step := range ct.Steps {
		success = t.Run(step.Executor.GetDescription(), func(t *testing.T) {
			...
			if step.SkipKubernetesOutputResourceValidation {
			...
			} else if step.MessagingRPResources == nil || len(step.MessagingRPResources.Resources) == 0 {
				...
				validation.ValidateMessagingRPResources(ctx, t, step.MessagingRPResources, ct.Options.ManagementClient)
	...
	for _, step := range ct.Steps {
		...
		if (step.MessagingRPResources == nil && step.SkipKubernetesOutputResourceValidation) || step.SkipResourceDeletion {
			continue
		}

		for _, resource := range step.MessagingRPResources.Resources {
			t.Logf("deleting %s", resource.Name)
			err := validation.DeleteMessagingRPResource(ctx, t, cli, ct.Options.ManagementClient, resource)
			require.NoErrorf(t, err, "failed to delete %s", resource.Name)
			t.Logf("finished deleting %s", ct.Description)
