...
func NewTestOptions(t *testing.T) TestOptions {
	...
	_, radiusConnection, err := kubernetes.CreateAPIServerConnection(az.Context, az.APIServerBaseURL, false)
	require.NoError(t, err, "failed to create API Server connection")

	radiusBaseURL, radiusRoundTripper, err := kubernetes.GetBaseUrlAndRoundTripper(az.APIServerBaseURL, "api.radius.dev", az.Context, false)
