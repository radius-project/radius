...
The deploy workflow Radius uses to deploy an application on demand against a user's target cluster. It stands up an ephemeral [k3d](https://k3d.io) control plane on the runner, restores persisted state, runs `rad deploy`, then persists state again and tears the control plane down. The deploy flow (see the `radius-deploy` skill) generates this file, commits it to the target repo as `.github/workflows/radius-deploy.yml`, and dispatches it.
...
2. **Connect to the target cluster.** Fetches a kubeconfig for the user's EKS/AKS cluster (`RADIUS_K8S_CLUSTER`). For EKS it ensures an access entry for the IAM role and writes a static, token-based kubeconfig. The path is exported as `RADIUS_TARGET_KUBECONFIG` for later steps.
...
8. **Deploy the application.** Ensures the `radiusCompute` extension is imported into `.radius/app.bicep`, then runs `rad deploy .radius/app.bicep` against the environment, passing the `image` parameter (the `image` input, defaulting to the commit SHA) and, when set, the `password` parameter from the `RADIUS_DB_PASSWORD` secret.
...
- **Trigger:** `workflow_dispatch` with an `environment` input (the GitHub Environment name) and an optional `image` input. The job binds to that environment via `environment: ${{ inputs.environment }}`.
- **Inputs:**

  | Input | Required | Description |
  |---|---|---|
  | `environment` | Yes | The GitHub Environment name, used as the Radius environment. |
  | `image` | No | Container image for the application, passed to `rad deploy` as the `image` parameter. Defaults to the commit SHA (`github.sha`) when unset. |
...
|---|---|---|
| Common | `RADIUS_K8S_CLUSTER` | `RADIUS_K8S_NAMESPACE` (default `default`), `RADIUS_BUILD_REGISTRY` (default `ghcr.io/<owner>`) |
| Azure | `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_RESOURCE_GROUP` | `AZURE_LOCATION` (default `eastus`) |
| AWS | `AWS_IAM_ROLE_ARN`, `AWS_REGION`, `AWS_ACCOUNT_ID`, `RADIUS_VPC_ID`, `RADIUS_SUBNET_IDS` | — |

The provider's branch runs only when its identifying variable (`AZURE_CLIENT_ID` or `AWS_IAM_ROLE_ARN`) is non-empty. When neither is set, resources deploy to the ephemeral control-plane cluster instead of an external target.
...
- The target cluster (`RADIUS_K8S_CLUSTER`) must already exist and be reachable; the assumed identity needs cluster-admin-level access to it.
