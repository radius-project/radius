...
- **Outputs:** a combined `rad-commands-result` artifact — a JSON document with a top-level `outcome`/`exitCode` and a `commands` array (one entry per command, in input order, with each command's exit code and output).
  ...
  The top-level `outcome` accumulator is pessimistic. It is seeded to `interrupted` and promoted to `succeeded` only after the command loop completes; every failure path assigns its own value (`command_failed`, `disallowed_command`) and exits first. The result file is written by an `EXIT` trap, which also fires when the step is killed by a `timeout-minutes` bound — in that case no failure branch ever runs, so an optimistic seed would publish an unearned success. `publish-deploy-status` maps `interrupted`, like any other unrecognized outcome, to a failed run state. A cancelled job kills the step the same way, but the deploy workflows gate `Publish deployed graph and status` on `!cancelled()`, so a cancellation deliberately publishes no status at all and the seed only corrects the uploaded `rad-commands-result` artifact.
