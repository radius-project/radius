...
- **Outputs:** a combined `rad-commands-result` artifact — a JSON document with a top-level `outcome`/`exitCode` and a `commands` array (one entry per command, in input order, with each command's exit code and output).
  ...
  The top-level `outcome` accumulator is pessimistic. It is seeded to `interrupted` and promoted to `succeeded` only once the command loop completes, and the promotion is guarded so it can only ever run from that seed. Every failure path assigns its own value (`command_failed`, `disallowed_command`) and exits. The result file is written by an `EXIT` trap, so any exit before the promotion publishes the seed rather than an unearned success. The reachable case today is an `errexit` abort — GitHub runs `shell: bash` steps as `bash --noprofile --norc -eo pipefail {0}`, so an unguarded non-zero command ends the step without any failure branch running — and the same holds for a step timeout or a cancellation. `publish-deploy-status` maps `interrupted`, like any other unrecognized outcome, to a failed run state. On a cancelled job the deploy workflows gate `Publish deployed graph and status` on `!cancelled()`, so no status is published at all; the corrected outcome then survives only in the `rad-commands-result` artifact, and only when the upload — which shares the step's deadline — still gets to run. A `SIGKILL` runs no trap and so produces no artifact.
...
`rad startup` and `rad shutdown` are kind-agnostic CLI commands that restore and back up all durable Radius state (control-plane PostgreSQL + Terraform recipe-state Secrets). These workflows use the OCI-backed state archive by default — the `RADIUS_STATE_*` variables select an OCI repository and the workflow logs in to GHCR before `rad startup`/`rad shutdown` — and fall back to the `radius-state` git orphan branch only when `RADIUS_STATE_BACKEND=git`. They do not manage cluster lifecycle — the workflow owns creating and destroying the ephemeral control plane around them. `rad startup` runs after the install (so `rad deploy` plans against prior state) and `rad shutdown` runs after the commands with `if: always()` (so state survives a failed deploy), but only persists when `rad startup` restored state earlier in the run — a run that fails before `rad startup` skips `rad shutdown` so it cannot overwrite the archive with uninitialized state.
