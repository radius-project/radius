...
11. **Register resource types and recipes.** Clones [resource-types-contrib](https://github.com/radius-project/resource-types-contrib), registers the `Radius.Compute/containerImages` type, builds its local Bicep extension (`radiuscompute.tgz`), and `rad deploy`s a generated `radius-env.bicep` that defines the provider-specific Terraform recipe pack (`aws-terraform` / `azure-terraform`) and the `Radius.Core/environments` resource.
