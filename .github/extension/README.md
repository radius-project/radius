...
14. **Publish deployed graph/status branch.** On successful command execution, the shared `publish-deploy-status` action runs `rad app graph --application <app> --preview --include-icons --output json` against the live control plane and publishes `deploy-graph.json` plus sibling status files (`deploy-progress.log`, `deploy-activity.log`, `deploy-controlplane.log`, `deploy-state.txt`) to the `radius-deploy-status` branch.
