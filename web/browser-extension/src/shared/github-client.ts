...
export class GitHubClient {
  ...
  async triggerVerification(
    ...
    const workflowExists = await this.fileExists(
      owner, repo, '.github/workflows/radius-verify-credentials.yml', branch,
    );
    if (!workflowExists) {
      await this.commitFile(
      ...
      // Wait for GitHub to index the new workflow.
      await delay(3000);
    }
      ...
      - name: Connect to EKS cluster
        ...
        run: |
          aws eks update-kubeconfig \\
            ...
            --kubeconfig "$HOME/.kube/target-cluster"
      ...
      - name: Configure external deployment target
        run: |
          ...
          PATCH_DE='[
            ...
            {"op":"add","path":"/spec/template/spec/volumes/-","value":{"name":"target-kubeconfig","secret":{"secretName":"target-kubeconfig"}}},
            {"op":"add","path":"/spec/template/spec/containers/0/volumeMounts/-","value":{"name":"target-kubeconfig","mountPath":"/root/.kube","readOnly":true}},
            {"op":"add","path":"/spec/template/spec/containers/0/env/-","value":{"name":"KUBECONFIG","value":"/root/.kube/config"}}
          ]'
          ...
          kubectl patch deployment bicep-de -n radius-system --type=json -p="$PATCH_DE"
