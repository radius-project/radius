...
func TestSqlDatabaseWithValues_ConvertVersionedToDataModel(t *testing.T) {
	// arrange
	rawPayload := loadTestData("sqldatabaseresourcewithvalues.json")
	versionedResource := &SQLDatabaseResource{}
	err := json.Unmarshal(rawPayload, versionedResource)
	require.NoError(t, err)
...
func TestSqlDatabaseWithValues_ConvertDataModelToVersioned(t *testing.T) {
	// arrange
	rawPayload := loadTestData("sqldatabaseresourcedatamodelwithvalues.json")
	resource := &datamodel.SqlDatabase{}
	err := json.Unmarshal(rawPayload, resource)
	require.NoError(t, err)
