...
func NewCreateOrUpdateExtender(ds store.StorageClient, sm manager.StatusManager) (ctrl.Controller, error) {
	return &CreateOrUpdateExtender{ctrl.NewBaseController(ds, sm)}, nil
}
...
func (extender *CreateOrUpdateExtender) Run(ctx context.Context, req *http.Request) (rest.Response, error) {
	...
	versioned, err := converter.ExtenderDataModelToVersioned(newResource, serviceCtx.APIVersion)
	if err != nil {
		return nil, err
	}
