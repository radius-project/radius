...
func NewListExtenders(ds store.StorageClient, sm manager.StatusManager, dp deployment.DeploymentProcessor) (ctrl.Controller, error) {
	return &ListExtenders{ctrl.NewBaseController(ds, sm, dp)}, nil
}
...
func (extender *ListExtenders) Run(ctx context.Context, req *http.Request) (rest.Response, error) {
	...
	result, err := extender.DataStore.Query(ctx, query, store.WithPaginationToken(serviceCtx.SkipToken), store.WithMaxQueryItemCount(serviceCtx.Top))
	if err != nil {
		return nil, err
	}
