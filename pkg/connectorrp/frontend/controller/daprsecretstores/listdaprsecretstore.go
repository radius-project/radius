...
func NewListDaprSecretStores(ds store.StorageClient, sm manager.StatusManager, dp deployment.DeploymentProcessor) (ctrl.Controller, error) {
	return &ListDaprSecretStores{ctrl.NewBaseController(ds, sm, dp)}, nil
}
...
func (daprSecretStore *ListDaprSecretStores) Run(ctx context.Context, req *http.Request) (rest.Response, error) {
	...
	result, err := daprSecretStore.DataStore.Query(ctx, query, store.WithPaginationToken(serviceCtx.SkipToken), store.WithMaxQueryItemCount(serviceCtx.Top))
	if err != nil {
		return nil, err
	}
