...
func NewListDaprPubSubBrokers(ds store.StorageClient, sm manager.StatusManager, dp deployment.DeploymentProcessor) (ctrl.Controller, error) {
	return &ListDaprPubSubBrokers{ctrl.NewBaseController(ds, sm, dp)}, nil
}
...
func (daprPubSub *ListDaprPubSubBrokers) Run(ctx context.Context, req *http.Request) (rest.Response, error) {
	...
	result, err := daprPubSub.DataStore.Query(ctx, query, store.WithPaginationToken(serviceCtx.SkipToken), store.WithMaxQueryItemCount(serviceCtx.Top))
	if err != nil {
		return nil, err
	}
