...
func NewListSecretsMongoDatabase(ds store.StorageClient, sm manager.StatusManager, dp deployment.DeploymentProcessor) (ctrl.Controller, error) {
	return &ListSecretsMongoDatabase{ctrl.NewBaseController(ds, sm, dp)}, nil
}
...
func (ctrl *ListSecretsMongoDatabase) Run(ctx context.Context, req *http.Request) (rest.Response, error) {
	...
	secrets, err := ctrl.DeploymentProcessor.FetchSecrets(ctx, rp.ResourceData{ID: sCtx.ResourceID, Resource: resource, OutputResources: resource.Properties.Status.OutputResources, ComputedValues: resource.ComputedValues, SecretValues: resource.SecretValues})
	if err != nil {
		return nil, err
	}
