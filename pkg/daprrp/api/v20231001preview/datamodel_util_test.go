...
func TestToScopesDataModel(t *testing.T) {
	testCases := []struct {
		scopes   []*string
		expected []string
	}{
		...
		{
			scopes:   []*string{to.Ptr("test")},
			expected: []string{"test"},
		},
		{
			scopes:   []*string{to.Ptr(""), to.Ptr("test")},
			expected: []string{"test"},
		},
		{
			scopes:   []*string{to.Ptr("test1"), to.Ptr("test2")},
			expected: []string{"test1", "test2"},
		},
	}
	for _, tt := range testCases {
		actual := toScopesDataModel(tt.scopes)
		require.Equal(t, tt.expected, actual)
	}
}

func TestFromScopesDataModel(t *testing.T) {
	testCases := []struct {
		scopes   []string
		expected []*string
	}{
		...
		},
		{
			...
			expected: []*string{to.Ptr("test1"), to.Ptr("test2")},
		},
	}

	for _, tt := range testCases {
		actual := fromScopesDataModel(tt.scopes)
		require.Equal(t, tt.expected, actual)
	}
}
