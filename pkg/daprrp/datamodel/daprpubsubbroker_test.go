...
func TestDaprPubSubBroker_ApplyDeploymentOutput(t *testing.T) {
	tests := []struct {
		name    string
		dss     *DaprPubSubBroker
	...
	}{
		{
			...
			do: &rpv1.DeploymentOutput{
				DeployedOutputResources: []rpv1.OutputResource{
					{
						LocalID:       rpv1.LocalIDDaprPubSubBrokerKafka,
						RadiusManaged: to.Ptr(true),
					},
				},
				ComputedValues: map[string]any{
					renderers.ComponentNameKey: "dapr-secret-store-test",
				},
			},
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.dss.ApplyDeploymentOutput(*tt.do); (err != nil) != tt.wantErr {
				t.Errorf("DaprPubSubBroker.ApplyDeploymentOutput() error = %v, wantErr %v", err, tt.wantErr)
			}
