...
func TestDaprSecretStore_ApplyDeploymentOutput(t *testing.T) {
	tests := []struct {
		name    string
		dss     *DaprSecretStore
	...
	}{
		{
			...
			do: &rpv1.DeploymentOutput{
				DeployedOutputResources: []rpv1.OutputResource{
					{
						LocalID:       rpv1.LocalIDDaprSecretStoreAzureKeyVault,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.dss.ApplyDeploymentOutput(*tt.do); (err != nil) != tt.wantErr {
				t.Errorf("DaprSecretStore.ApplyDeploymentOutput() error = %v, wantErr %v", err, tt.wantErr)
			}
