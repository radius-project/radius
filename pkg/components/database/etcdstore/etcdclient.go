...
var _ database.Client = (*ETCDClient)(nil)
...
func (c *ETCDClient) Query(ctx context.Context, query database.Query, options ...database.QueryOptions) (*database.ObjectQueryResult, error) {
	if ctx == nil {
...
func (c *ETCDClient) Get(ctx context.Context, id string, options ...database.GetOptions) (*database.Object, error) {
	if ctx == nil {
		return nil, &database.ErrInvalid{Message: "invalid argument. 'ctx' is required"}
	}
...
func (c *ETCDClient) Delete(ctx context.Context, id string, options ...database.DeleteOptions) error {
	if ctx == nil {
...
func (c *ETCDClient) Save(ctx context.Context, obj *database.Object, options ...database.SaveOptions) error {
	if ctx == nil {
		return &database.ErrInvalid{Message: "invalid argument. 'ctx' is required"}
	}
	if obj == nil {
...
func keyFromID(id resources.ID) string {
	prefix, rootScope, routingScope, _ := databaseutil.ExtractStorageParts(id)
...
func keyFromQuery(query database.Query) string {
	...
	prefix := databaseutil.ResourcePrefix
	if query.IsScopeQuery {
		prefix = databaseutil.ScopePrefix
	}

	if query.ScopeRecursive {
		return prefix + SectionSeparator + databaseutil.NormalizePart(query.RootScope)
	} else {
		return prefix + SectionSeparator + databaseutil.NormalizePart(query.RootScope) + SectionSeparator + databaseutil.NormalizePart(query.RoutingScopePrefix)
	}
}

func keyMatchesQuery(key []byte, query database.Query) bool {
