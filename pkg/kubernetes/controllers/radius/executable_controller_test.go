...
var (
	...
	executor *localenvtest.TestProcessExecutor
	kclient  runtimeclient.Client
)
...
func startController() error {
	...
	kclient, err = runtimeclient.New(cfg, runtimeclient.Options{
		Scheme: scheme,
	})
	if err != nil {
...
func ensureNamespace(ctx context.Context, namespace string) error {
	...
	return kclient.Create(ctx, &nsObject, &runtimeclient.CreateOptions{})
}

func ensureReplicasRunning(ctx context.Context, cmdPath string, n int) error {
	return executor.WaitProcessesStarted(ctx, cmdPath, n)
}
...
func updateExecutable(t *testing.T, ctx context.Context, key runtimeclient.ObjectKey, applyChanges func(*radiusv1alpha3.Executable)) error {
	...
	for {
		...
		if err := kclient.Get(ctx, key, &exe); err != nil {
			return err
		}

		applyChanges(&exe)

		if err := kclient.Update(ctx, &exe); err != nil {
...
func TestExecutableStartsReplicas(t *testing.T) {
	...
	if err := kclient.Create(ctx, &exe, &runtimeclient.CreateOptions{}); err != nil {
...
func TestExitCodeCaptured(t *testing.T) {
	...
	waitExitCodeCaptured := func() (bool, error) {
		...
		if err := kclient.Get(ctx, runtimeclient.ObjectKeyFromObject(&exe), &updatedExe); err != nil {
...
func TestExecutableFinishAfterScaleDown(t *testing.T) {
	...
	replicas = executor.FindAll(exe.Spec.Executable, func(pe localenvtest.ProcessExecution) bool {
		return !pe.EndedAt.IsZero() && pe.ExitCode == 0
	})
	require.Equal(t, 2, len(replicas), "Expected two normally finished replicas")

	replicas = executor.FindAll(exe.Spec.Executable, func(pe localenvtest.ProcessExecution) bool {
		return !pe.EndedAt.IsZero() && pe.ExitCode == localenvtest.KilledProcessExitCode
	})
...
func TestReplicasTerminatedUponExecutableDeletion(t *testing.T) {
	...
	if err := kclient.Delete(ctx, &exe); err != nil {
		t.Fatalf("Unable to create Executable: %v", err)
	}

	t.Log("Waiting for all replicas to be killed...")
	waitReplicasKilled := func() (bool, error) {
		killedReplicas := executor.FindAll(exe.Spec.Executable, func(pe localenvtest.ProcessExecution) bool {
			return !pe.EndedAt.IsZero() && pe.ExitCode == localenvtest.KilledProcessExitCode
		})
