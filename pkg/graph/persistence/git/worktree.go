...
// Package git provides a persistence.Store backed by an orphan branch in a
// git repository. State is written through a git worktree so that the orphan
// branch files never pollute the application working tree.
//
// This package is composed of two layers:
//
//   - worktree.go : low-level StateWorktree primitive (OpenOrCreate / WriteFile
//     / CommitAndPush / Remove). Adapted from the
//     filesystem-state branch implementation.
//   - git_store.go: Store implementation that maps persistence.Key values to
//     files on the orphan branch and uses StateWorktree for I/O.
package git
...
// StateWorktree is a git worktree checked out to an orphan branch in a
// temporary directory that is completely isolated from the application
// working tree.
//
// Lifecycle:
...
// OpenOrCreate opens a git worktree for branchName, creating the orphan branch
// if it does not yet exist, and returns the worktree. The worktree is placed
...
// Remove tears down the worktree entry. Always defer this after a successful
// OpenOrCreate.
...
// push pushes the state branch to origin. A missing remote is treated as a
// non-fatal condition for local development and unit tests, but any push
// failure with an existing origin is returned so CI cannot report success for
// a stale artifact.
...
// createOrphanBranch creates an empty orphan branch using low-level git
// plumbing without touching the current working tree or switching branches.
// It relies on the well-known empty-tree SHA that is constant across all git
// repositories.
