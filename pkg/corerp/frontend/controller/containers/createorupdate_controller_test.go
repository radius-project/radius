...
func TestCreateOrUpdateContainer_20220315PrivatePreview(t *testing.T) {
	...
	createNewResourceCases := []struct {
		...
		{"create-new-resource-etag-if-match", "If-Match", "random-etag", "", 412, true},
		{"create-new-resource-*-if-none-match", "If-None-Match", "*", "", 200, false},
	}

	for _, tt := range createNewResourceCases {
		t.Run(tt.desc, func(t *testing.T) {
			input, dataModel, expectedOutput := getTestModels_20220315privatepreview()
			...
			ctl, err := NewCreateOrUpdateController(mStorageClient, nil)
	...
	for _, tt := range patchNonExistingResourceCases {
		t.Run(fmt.Sprint(tt.desc), func(t *testing.T) {
			input, _, _ := getTestModels_20220315privatepreview()
			w := httptest.NewRecorder()
			req, _ := radiustesting.GetARMTestHTTPRequest(ctx, http.MethodPatch, testHeaderfile, input)
			req.Header.Set(tt.headerKey, tt.headerValue)
			ctx := radiustesting.ARMTestContextFromRequest(req)
