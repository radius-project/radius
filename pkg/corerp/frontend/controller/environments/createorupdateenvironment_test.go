...
func TestCreateOrUpdateRecipesWithParameters(t *testing.T) {
	mctrl := gomock.NewController(t)
	defer mctrl.Finish()

	mStorageClient := store.NewMockStorageClient(mctrl)
	ctx := context.Background()
	t.Run("Create recipes with parameters successfully", func(t *testing.T) {
		envInput, envDataModel, expectedOutput := getTestModelsWithRecipeParameters20220315privatepreview()
		w := httptest.NewRecorder()
		req, _ := radiustesting.GetARMTestHTTPRequest(ctx, http.MethodGet, testHeaderfile, envInput)
		ctx := radiustesting.ARMTestContextFromRequest(req)
