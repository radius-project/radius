...
func (e *PatchEnvironment) Run(ctx context.Context, req *http.Request) (rest.Response, error) {
	...
	updatedResource, err := e.Validate(ctx, req)
	if err != nil {
		return nil, err
	}
	...
	ctrl.UpdateExistingResourceData(ctx, existingResource, updatedResource)

	ur, err := e.SaveResource(ctx, serviceCtx.ResourceID.ID, updatedResource, etag)
	if err != nil {
		return nil, err
	}

	versioned, err := converter.EnvironmentDataModelToVersioned(updatedResource, serviceCtx.APIVersion)
	if err != nil {
		return nil, err
	}
...
func (e *PatchEnvironment) Validate(ctx context.Context, req *http.Request) (*datamodel.Environment, error) {
	serviceCtx := servicecontext.ARMRequestContextFromContext(ctx)
	...
	dm, err := converter.EnvironmentDataModelFromVersioned(content, serviceCtx.APIVersion)
	if err != nil {
		return nil, err
	}
