	...
	for _, tt := range createCases {
		t.Run(tt.desc, func(t *testing.T) {
			...
			if tt.rErr != nil {
				...
				locationHeader := getAsyncLocationPath(sCtx.ResourceID, gatewayDataModel.TrackedResource.Location, "operationResults", sCtx.OperationID)
				require.NotNil(t, w.Header().Get("Location"))
				require.Equal(t, locationHeader, w.Header().Get("Location"))

				azureAsyncOpHeader := getAsyncLocationPath(sCtx.ResourceID, gatewayDataModel.TrackedResource.Location, "operationStatuses", sCtx.OperationID)
