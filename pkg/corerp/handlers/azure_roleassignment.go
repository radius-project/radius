...
func (handler *azureRoleAssignmentHandler) Put(ctx context.Context, options *PutOptions) (map[string]string, error) {
	...
	roleAssignment, err := roleassignment.Create(ctx, parsedScope.FindScope(resources.SubscriptionsSegment), principalID, scope, roleName)
	if err != nil {
	...
	options.Resource.Identity = resourcemodel.NewARMIdentity(&options.Resource.ResourceType, *roleAssignment.ID, clientv2.GetAPIVersionFromUserAgent(authorization.UserAgent()))
...
func (handler *azureRoleAssignmentHandler) Delete(ctx context.Context, options *DeleteOptions) error {
	...
	return roleassignment.Delete(ctx, handler.arm.TokenCredential, roleID)
}
