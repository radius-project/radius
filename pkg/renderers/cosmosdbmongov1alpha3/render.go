...
func (r Renderer) Render(ctx context.Context, resource renderers.RendererResource, dependencies map[string]renderers.RendererDependency) (renderers.RendererOutput, error) {
	properties := CosmosDBMongoComponentProperties{}
...
func RenderManaged(name string, properties CosmosDBMongoComponentProperties) ([]outputresource.OutputResource, error) {
...
func RenderUnmanaged(name string, properties CosmosDBMongoComponentProperties) ([]outputresource.OutputResource, error) {
	if properties.Resource == "" {
		return nil, renderers.ErrResourceMissingForUnmanagedResource
	}

	databaseID, err := renderers.ValidateResourceID(properties.Resource, MongoResourceType, "CosmosDB Mongo Database")
	if err != nil {
		return nil, err
	}
