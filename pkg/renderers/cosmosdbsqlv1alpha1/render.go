...
func (r Renderer) AllocateBindings(ctx context.Context, workload workloads.InstantiatedWorkload, resources []workloads.WorkloadResourceProperties) (map[string]components.BindingState, error) {
	...
	if len(resources) != 1 || resources[0].Type != resourcekinds.KindAzureCosmosDBSQL {
		return nil, fmt.Errorf("cannot fulfill service - expected properties for %s", resourcekinds.KindAzureCosmosDBSQL)
	}
...
func (r Renderer) Render(ctx context.Context, w workloads.InstantiatedWorkload) ([]outputresource.OutputResource, error) {
	...
	if component.Config.Managed {
		...
		resource := outputresource.OutputResource{
			Kind:    resourcekinds.KindAzureCosmosDBSQL,
