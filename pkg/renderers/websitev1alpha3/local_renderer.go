...
func (r *LocalRenderer) Render(ctx context.Context, resource renderers.RendererResource, dependencies map[string]renderers.RendererDependency) (renderers.RendererOutput, error) {
	website, err := convert(resource)
	if err != nil {
	...
	executable, err := r.makeExecutable(ctx, resource, dependencies, website)
	if err != nil {
		return renderers.RendererOutput{}, err
	}
