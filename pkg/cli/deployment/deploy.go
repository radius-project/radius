...
type ResourceDeploymentClient struct {
	...
	DeploymentsClient          clientv2.DeploymentsClient
	DeploymentOperationsClient clientv2.DeploymentOperationsClient
...
func (dc *ResourceDeploymentClient) startDeployment(ctx context.Context, name string, options clients.DeploymentOptions) (*runtime.Poller[clientv2.ClientCreateOrUpdateResponse], error) {
	...
	poller, err := dc.DeploymentsClient.BeginCreateOrUpdate(ctx,
		clientv2.Deployment{
		...
		clientv2.NewClientBeginCreateOrUpdateOptions(resourceId, "", ""),
	)
...
func (dc *ResourceDeploymentClient) listOperations(ctx context.Context, deploymentName string) ([]armresources.DeploymentOperation, error) {
	var resourceId string
	...
	operationList, err := dc.DeploymentOperationsClient.List(ctx, dc.RadiusResourceGroup, deploymentName, resourceId, nil)
	if err != nil {
		return nil, err
	}
	...
	for operationList.NextLink != nil {
		...
		operations = append(operations, *operationList.Value...)
	}
