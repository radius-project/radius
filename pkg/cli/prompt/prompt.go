...
type BubbleTeaInterface interface {
	GetInput(program tea.Program) (string, error)

	RunSelect(items []string, label string) (int, string, error)
}

type BubbleTeaImpl struct{}

func (i *BubbleTeaImpl) GetInput(program tea.Program) (string, error) {
	// TODO: implement text model
	return "", nil
}

func (i *BubbleTeaImpl) RunSelect(items []string, label string) (int, string, error) {
	lm := cli_list.NewListModel(items, label)
