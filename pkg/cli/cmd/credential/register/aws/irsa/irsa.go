...
`,
	...
	cmd.Flags().String("iam-role", "", "RoleARN for AWS IRSA identity.")
	_ = cmd.MarkFlagRequired("iam-role")
...
func (r *Runner) Validate(cmd *cobra.Command, args []string) error {
	...
	iamRole, err := cmd.Flags().GetString("iam-role")
	if err != nil {
		return err
	}
	...
	r.IAMRole = iamRole
	if r.IAMRole == "" {
		return clierrors.Message("IAM Role %q cannot be empty.", r.IAMRole)
	}
...
// Run runs the `rad credential register aws irsa` command.
//
