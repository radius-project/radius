...
func (r *Runner) injectAutomaticParameters(template map[string]any) error {
	...
	if r.Providers.Radius.ApplicationID != "" {
		err := bicep.InjectApplicationParam(template, r.Parameters, r.Providers.Radius.EnvironmentID)
		if err != nil {
			return err
		}
	}
...
func (r *Runner) reportMissingParameters(template map[string]any) error {
	parameters, err := bicep.ExtractParameters(template)
	if err != nil {
		return err
	}
	...
	for parameter := range parameters {
		if _, ok := r.Parameters[parameter]; ok {
			// Has user-provided value
			continue
		}

		if _, ok := bicep.DefaultValue(parameters[parameter]); ok {
			// Has default value
			continue
		}
