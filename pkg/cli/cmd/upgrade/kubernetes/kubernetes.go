...
`,
	...
	cmd.Flags().BoolVar(&runner.DryRun, "dry-run", false, "Simulate the upgrade without making changes")
...
Dry-run handling.
• If --dry-run is set, simulate the upgrade using Helm’s dry-run option and output a report, then exit.
...
• Honor the dry-run flag if needed.
