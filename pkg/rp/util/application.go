...
func FetchApplication(ctx context.Context, application string, ucpOptions *arm.ClientOptions) (*v20230415preview.ApplicationResource, error) {
	applicationID, err := resources.ParseResource(application)
	if err != nil {
		return nil, err
	}

	client, err := v20230415preview.NewApplicationsClient(applicationID.RootScope(), &aztoken.AnonymousCredential{}, ucpOptions)
	if err != nil {
		return nil, err
	}
