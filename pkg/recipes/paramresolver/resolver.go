...
// It traverses the parameter map recursively and replaces expressions with values from the context lookup.
// Unrecognized expressions are left as-is per R-011.
...
func resolveString(s string, lookup map[string]string) string {
	return expressionPattern.ReplaceAllStringFunc(s, func(match string) string {
		...
		// Unrecognized expression — leave as-is per R-011.
		return match
	})
}
...
// Returns the resolved result and true if the expression is a valid ternary, or ("", false) otherwise.
// If the condition path cannot be resolved, the entire ternary is left as-is.
func evaluateTernary(inner string, lookup map[string]string) (string, bool) {
	...
	if !ok {
		// Unresolvable condition — leave entire expression as-is.
