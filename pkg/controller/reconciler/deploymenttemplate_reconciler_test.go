...
func Test_DeploymentTemplateReconciler_ChangeEnvironmentAndApplication(t *testing.T) {
	ctx := testcontext.New(t)
	radius, client := SetupDeploymentTemplateTest(t)

	name := types.NamespacedName{Namespace: "DeploymentTemplate-change", Name: "test-DeploymentTemplate-change"}
	err := client.Create(ctx, &corev1.Namespace{ObjectMeta: ctrl.ObjectMeta{Name: name.Namespace}})
	require.NoError(t, err)
	...
	// Deployment will update after operation completes
	status = waitForDeploymentTemplateStateReady(t, client, name)
	require.Equal(t, "/planes/radius/local/resourcegroups/new-environment-new-application/providers/Microsoft.Resources/deployments/test-DeploymentTemplate-change", status.Resource)
