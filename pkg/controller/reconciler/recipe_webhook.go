...
func (r *RecipeWebhook) ValidateCreate(ctx context.Context, obj *radappiov1alpha3.Recipe) (admission.Warnings, error) {
	logger := ucplog.FromContextOrDiscard(ctx)

	logger.Info("Validating Create Recipe %s", obj.Name)
	return r.validateRecipeType(ctx, obj)
}

// ValidateUpdate validates the update of a Recipe object.
func (r *RecipeWebhook) ValidateUpdate(ctx context.Context, oldObj, newObj *radappiov1alpha3.Recipe) (admission.Warnings, error) {
	logger := ucplog.FromContextOrDiscard(ctx)

	logger.Info("Validating Update Recipe %s", newObj.Name)
	return r.validateRecipeType(ctx, newObj)
}

// ValidateDelete validates the deletion of a Recipe object.
func (r *RecipeWebhook) ValidateDelete(ctx context.Context, obj *radappiov1alpha3.Recipe) (admission.Warnings, error) {
	logger := ucplog.FromContextOrDiscard(ctx)
