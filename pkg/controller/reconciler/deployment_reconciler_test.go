...
func Test_DeploymentReconciler_StartDeleteOperationIfNeeded_OwnershipMismatch_BlocksDelete(t *testing.T) {
	...
	deployment := &appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{Name: "legitimate-app", Namespace: "attacker"}}
	containerID := "/planes/radius/local/resourceGroups/tenant-b/providers/Applications.Core/containers/victim-container"
		...
		radius.containers[containerID] = v20231001preview.ContainerResource{
			Properties: &v20231001preview.ContainerProperties{
				Resources: []*v20231001preview.ResourceReference{{
					ID: new("/planes/kubernetes/local/namespaces/victim/providers/apps/Deployment/victim-app"),
				}},
			},
		}
	})
	...
	_, err = radius.Containers("/planes/radius/local/resourceGroups/tenant-b").Get(ctx, "victim-container", nil)
	require.NoError(t, err)
}
...
func Test_DeploymentReconciler_StartPutOrDeleteOperationIfNeeded_OwnershipMismatch_BlocksDelete(t *testing.T) {
	...
	deployment := &appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{Name: "legitimate-app", Namespace: "attacker"}}
	victimContainerID := "/planes/radius/local/resourceGroups/tenant-b/providers/Applications.Core/containers/victim-container"

	radius.Update(func() {
		radius.containers[victimContainerID] = v20231001preview.ContainerResource{
			Properties: &v20231001preview.ContainerProperties{
				Resources: []*v20231001preview.ResourceReference{{
					ID: new("/planes/kubernetes/local/namespaces/victim/providers/apps/Deployment/victim-container"),
				}},
			},
		}
	})
	...
	annotations := &deploymentAnnotations{
		...
		Status: &deploymentStatus{
			...
			Application: "/planes/radius/local/resourceGroups/tenant-a/providers/Applications.Core/applications/legitimate-app",
			Container:   victimContainerID,
		},
	}
	...
	requireNoDeleteOperation(t, radius, victimContainerID)
...
func Test_DeploymentReconciler_StartDeleteOperationIfNeeded_OwnershipMatch_AllowsDelete(t *testing.T) {
	...
	deployment := &appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{Name: "legitimate-app", Namespace: "attacker"}}
	containerID := "/planes/radius/local/resourceGroups/tenant-a/providers/Applications.Core/containers/legitimate-app"

	radius.Update(func() {
		radius.containers[containerID] = v20231001preview.ContainerResource{
			Properties: &v20231001preview.ContainerProperties{
				Resources: []*v20231001preview.ResourceReference{{
					ID: new(makeKubernetesDeploymentResourceID("attacker", "legitimate-app")),
				}},
			},
		}
	})
