...
func (c *TrackedResourceProcessController) Run(ctx context.Context, request *ctrl.Request) (ctrl.Result, error) {
	client, err := c.DataProvider().GetStorageClient(ctx, v20231001preview.ResourceType)
	if err != nil {
		return ctrl.Result{}, err
	}

	resource, err := store.GetResource[datamodel.GenericResource](ctx, client, request.ResourceID)
	...
	downstreamURL, err := resourcegroups.ValidateDownstream(ctx, c.DataProvider(), originalID, v1.LocationGlobal, resource.Properties.APIVersion)
