...
var _ store.StorageClient = (*PostgresClient)(nil)
...
END AS result;`
	...
	args := []any{storeutil.NormalizePart(converted.String())}
...
END AS result;`
		...
		args = []any{storeutil.NormalizePart(converted.String()), etag}
	}
...
func (p *PostgresClient) Get(ctx context.Context, id string, options ...store.GetOptions) (*store.Object, error) {
	...
	err = p.api.QueryRow(
		...
		storeutil.NormalizePart(converted.String())).Scan(&obj.ID, &obj.ETag, &obj.Data)
...
func (p *PostgresClient) Query(ctx context.Context, query store.Query, options ...store.QueryOptions) (*store.ObjectQueryResult, error) {
	...
	if query.IsScopeQuery {
		...
		resourceType = storeutil.NormalizePart(resourceType)
	}

	var routingScopePrefixFilter *string
	if query.RoutingScopePrefix != "" {
		routingScopePrefixFilter = to.Ptr(storeutil.NormalizePart(query.RoutingScopePrefix))
	}
	...
	if config.PaginationToken != "" {
		...
		if err != nil {
			return nil, &store.ErrInvalid{Message: "invalid argument. 'query.PaginationToken' is invalid."}
		}
		timestampFilter = &ts
	}
	...
	if query.IsScopeQuery {
		var err error
		query.ResourceType, err = storeutil.ConvertScopeTypeToResourceType(query.ResourceType)
		if err != nil {
			return nil, err
		}
	}
...
LIMIT $6`
	...
	args := []any{
		...
		storeutil.NormalizePart(query.RootScope),
...
func (p *PostgresClient) Save(ctx context.Context, obj *store.Object, options ...store.SaveOptions) error {
	...
	if parsed.IsEmpty() {
		return &store.ErrInvalid{Message: "invalid argument. 'obj.ID' must not be empty"}
	}
	if parsed.IsResourceCollection() || parsed.IsScopeCollection() {
		return &store.ErrInvalid{Message: "invalid argument. 'obj.ID' must refer to a named resource, not a collection"}
	}
...
END AS result;`
	...
	args := []any{
		storeutil.NormalizePart(converted.String()),
		obj.ID, // MUST NOT BE NORMALIZED. Preserve the original casing and format.
		storeutil.NormalizePart(converted.Type()),
		storeutil.NormalizePart(converted.RootScope()),
		storeutil.NormalizePart(converted.RoutingScope()),
		obj.ETag,
		obj.Data,
	}
...
END AS result;`
		...
		args = []any{storeutil.NormalizePart(converted.String()), obj.Data, config.ETag}
	}
	...
	if err != nil {
		...
		return &store.ErrNotFound{ID: obj.ID}
	} else if result == "ErrConcurrency" {
		return &store.ErrConcurrency{}
	}
