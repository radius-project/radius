...
func Test_ValidateDownstream(t *testing.T) {
	...
	setup := func(t *testing.T) (*dataprovider.MockDataStorageProvider, *store.MockStorageClient) {
		ctrl := gomock.NewController(t)

		storageProvider := dataprovider.NewMockDataStorageProvider(ctrl)
		client := store.NewMockStorageClient(ctrl)

		storageProvider.EXPECT().GetStorageClient(gomock.Any(), gomock.Any()).Return(client, nil).AnyTimes()
	...
	t.Run("success (resource group)", func(t *testing.T) {
		...
		provider, client := setup(t)
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(&store.Object{Data: plane}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), id.RootScope()).Return(&store.Object{Data: resourceGroup}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), resourceTypeResource.ID).Return(&store.Object{Data: resourceTypeResource}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), locationResource.ID).Return(&store.Object{Data: locationResource}, nil).Times(1)

		expectedURL, err := url.Parse(downstream)
		require.NoError(t, err)

		downstreamURL, err := ValidateDownstream(testcontext.New(t), provider, id, location, apiVersion)
		require.NoError(t, err)
	...
	t.Run("success (non resource group)", func(t *testing.T) {
		provider, client := setup(t)
		client.EXPECT().Get(gomock.Any(), idWithoutResourceGroup.PlaneScope()).Return(&store.Object{Data: plane}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), resourceTypeResource.ID).Return(&store.Object{Data: resourceTypeResource}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), locationResource.ID).Return(&store.Object{Data: locationResource}, nil).Times(1)
		...
		downstreamURL, err := ValidateDownstream(testcontext.New(t), provider, idWithoutResourceGroup, location, apiVersion)
		require.NoError(t, err)
		require.Equal(t, expectedURL, downstreamURL)
	})
	...
	t.Run("success (operationstatuses as child resource)", func(t *testing.T) {
		...
		downstreamURL, err := ValidateDownstream(testcontext.New(t), provider, operationStatusID, location, apiVersion)
		require.NoError(t, err)
		require.Equal(t, expectedURL, downstreamURL)
	})
	...
	t.Run("success (operationresults as child resource)", func(t *testing.T) {
		...
		downstreamURL, err := ValidateDownstream(testcontext.New(t), provider, operationResultID, location, apiVersion)
		require.NoError(t, err)
		require.Equal(t, expectedURL, downstreamURL)
	})
	...
	t.Run("plane not found", func(t *testing.T) {
		provider, client := setup(t)
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(nil, &store.ErrNotFound{}).Times(1)
	...
	t.Run("plane retreival failure", func(t *testing.T) {
		...
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(nil, errors.New("test error")).Times(1)
	...
	t.Run("resource group not found", func(t *testing.T) {
		provider, client := setup(t)
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(&store.Object{Data: plane}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), id.RootScope()).Return(nil, &store.ErrNotFound{}).Times(1)
	...
	t.Run("resource group err", func(t *testing.T) {
		...
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(&store.Object{Data: plane}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), id.RootScope()).Return(nil, errors.New("test error")).Times(1)
	...
	t.Run("resource type error", func(t *testing.T) {
		...
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(&store.Object{Data: plane}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), id.RootScope()).Return(&store.Object{Data: resourceGroup}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), resourceTypeResource.ID).Return(nil, errors.New("test error")).Times(1)
	...
	t.Run("location error", func(t *testing.T) {
		...
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(&store.Object{Data: plane}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), id.RootScope()).Return(&store.Object{Data: resourceGroup}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), resourceTypeResource.ID).Return(&store.Object{Data: resourceTypeID}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), locationResource.ID).Return(nil, errors.New("test error")).Times(1)
...
func Test_ValidateDownstream_Legacy(t *testing.T) {
	...
	t.Run("success (resource group)", func(t *testing.T) {
		...
		client.EXPECT().Get(gomock.Any(), id.PlaneScope()).Return(&store.Object{Data: plane}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), id.RootScope()).Return(&store.Object{Data: resourceGroup}, nil).Times(1)
		client.EXPECT().Get(gomock.Any(), resourceTypeID.String()).Return(nil, &store.ErrNotFound{}).Times(1)
