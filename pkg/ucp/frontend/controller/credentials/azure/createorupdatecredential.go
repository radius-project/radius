...
func (p *CreateOrUpdateCredential) Run(ctx context.Context, w http.ResponseWriter, req *http.Request) (armrpc_rest.Response, error) {
	...
	newResource, err := converter.AzureCredentialDataModelFromVersioned(body, apiVersion)
	...
	existingResource := datamodel.AzureCredential{}
	...
	// Do not save the secret in the metadata store.
	...
	versioned, err := converter.AzureCredentialDataModelToVersioned(newResource, apiVersion)
	if err != nil {
		return nil, err
	}
