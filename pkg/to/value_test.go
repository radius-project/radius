...
func TestStringSlice(t *testing.T) {
	...
	w := []string{"a", "b"}
	require.Exactly(t, w, *new(w))
}

func TestStringMap(t *testing.T) {
	msp := map[string]*string{"foo": new("foo"), "bar": new("bar"), "baz": new("baz")}
...
func TestStringMapHandlesNil(t *testing.T) {
	msp := map[string]*string{"foo": new("foo"), "bar": nil, "baz": new("baz")}
	for k, v := range StringMap(msp) {
		if msp[k] != nil {
			require.Exactly(t, *msp[k], v)
		}
