...
func TestCreateOrUpdateRedisCache_20220315PrivatePreview(t *testing.T) {
	...
	_, deploymentOutput := getDeploymentProcessorOutputs(false)
	...
	for _, testcase := range createNewResourceTestCases {
		t.Run(testcase.desc, func(t *testing.T) {
			if testcase.azureResource {
				_, deploymentOutput = getDeploymentProcessorOutputs(true)
			}
			...
			if !testcase.shouldFail {
				mDeploymentProcessor.EXPECT().RenderAndDeploy(gomock.Any(), gomock.Any(), gomock.Any()).Times(1).Return(deploymentOutput, nil)
