...
func (daprStateStore *CreateOrUpdateDaprStateStore) Run(ctx context.Context, w http.ResponseWriter, req *http.Request) (rest.Response, error) {
	...
	deploymentOutput, err := daprStateStore.DeploymentProcessor().Deploy(ctx, serviceCtx.ResourceID, rendererOutput, newResource)
	if err != nil {
		return nil, err
	}

	newResource.Properties = deploymentOutput.RadiusResource.(*datamodel.DaprStateStore).Properties
