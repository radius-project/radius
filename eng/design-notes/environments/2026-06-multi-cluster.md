...
(`src/shared/github-client.ts`, `VERIFY_WORKFLOW` / `DEPLOY_WORKFLOW`). Those
workflows are what merge to `main` and what Repo Radius depends on, so the seam
Radius ships first must match the contract they already use. Where this design
diverges from the feature spec, [Divergence from the feature
...
- **End-to-end (the verify/deploy workflows themselves):** the primary v1
  ...
  running green against Radius `main`. They spin up an ephemeral k3d
...
### v1 (required for the Repo Radius verify/deploy workflows on `main`)
...
3. **Keep the `verify`/`deploy` workflows green** against `main` as the
...
- **Aligned to the prototype (decided):** the technical design follows the Repo
  ...
  merge to `main` and that Repo Radius depends on, rather than the feature spec's
  proposed mechanism. v1 honors an injected kubeconfig
  (`RADIUS_TARGET_KUBECONFIG` / `KUBE_CONFIG_PATH`); the feature spec's
  `eksClusterName` / `aksClusterName` named-cluster UX and in-process
  cloud-derived credential acquisition are v2 behind the same resolver seam. The
  feature spec was authored from a product perspective; see
