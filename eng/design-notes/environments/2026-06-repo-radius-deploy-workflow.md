...
| `rad_commands` | No       | A single `rad` CLI command string, **or** a JSON-encoded array of command strings run in order, with the `rad` prefix omitted (e.g. `deploy .radius/app.bicep --environment dev` or `["deploy .radius/app.bicep --environment dev", "app graph my-app -o json"]`). Each command is validated against the allowed-command set before any command runs. When empty, the workflow runs its default `rad deploy` of `.radius/app.bicep`. |
...
flowchart TD
    ...
    H --> I[Refresh EKS token<br/>then run rad_commands or default deploy<br/>upload rad-commands-result artifact]
