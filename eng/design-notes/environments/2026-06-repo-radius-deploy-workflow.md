...
- **The state-storage mechanism** (`rad startup` / `rad shutdown`, the OCI-backed state archive) — documented in the [current archive architecture](../../../docs/architecture/state-archive.md).
...
The run produces a single `rad-commands-result` artifact: a JSON document with a top-level `outcome` (`succeeded`, `command_failed`, `disallowed_command`, or `interrupted`) and `exitCode`, plus a `commands` array with one entry per command, in input order, each carrying the command string, its `exitCode`, `outcome`, and captured `output`. The artifact name is stable and known in advance, so the frontend downloads one file to get the full result of the run and reads the run conclusion from the matching exit code. The artifact is written even when a command fails (a trap finalizes it on exit), so a failed or disallowed run still yields a complete result; entries for commands that did not run are absent. `interrupted` is the seeded outcome, reported when the step exited after installing the trap but before reaching normal completion — a catchable termination such as a step timeout or cancellation, or an `errexit` abort. It is not a guarantee that every killed run produces an artifact: a `SIGKILL`, a lost runner, or a cancellation that outruns the artifact upload leaves no result at all, which the frontend sees as the `unknown` case. On failure, additional control-plane and application logs upload as the `radius-logs` artifact.
...
- The **composite actions** (`setup-control-plane`, `restore-state`, `run-rad-commands`, `teardown`) hold the provider-agnostic phases and are referenced from `radius-project/radius` at a pinned ref (the `{{RADIUS_REF}}` placeholder the generator fills — `main` once these actions merge there). `run-rad-commands` and `teardown` are deliberately separate: the provider workflow invokes `run-rad-commands` on success but `teardown` with `if: always()`, so `rad shutdown`, log collection, and the k3d delete still run when an earlier step fails without the deploy itself running after a failed prerequisite. Because composite-action steps cannot read the `vars`/`secrets` contexts or the caller's `env:` block, every value they need (namespace, app file, image, rad-commands, deploy params, registry credentials) is passed explicitly as an action input; the target-cluster kubeconfig path is the exception, shared via `RADIUS_TARGET_KUBECONFIG` exported to `$GITHUB_ENV`.

All **third-party actions** used anywhere in the generated workflows and shared composite actions (`actions/checkout`, `actions/upload-artifact`, `aws-actions/configure-aws-credentials`, `azure/login`, `azure/use-kubelogin`, `oras-project/setup-oras`, `hashicorp/setup-terraform`) are pinned to full commit SHAs with a trailing `# vX` comment rather than floating version tags, so a moved tag can't change what runs in a user's environment. Only the first-party `radius-project/radius` composite actions are referenced by ref (`{{RADIUS_REF}}`), since they are Radius-owned and versioned with the deploy workflow.
...
flowchart TD
    ...
    K --> KA[Apply custom recipe packs<br/>register custom types + attach authored pack<br/>optional]
    KA --> L[Provision registry creds on control plane]
...
- **Azure** — a GitHub Actions OIDC JWT expires after 5 minutes, so one projected assertion cannot cover a longer run. Two independent problems follow, and both are fixed. First, the mounted file goes stale: an Opaque Secret volume never mints a new assertion, so the shared command and delete actions re-mint it, update the `azure-oidc-token` Secret, and restart the three consumers before an operation, then keep updating the Secret every two minutes while it runs (kubelet propagates each update to the non-`subPath` mounts). Second, the reader caches: azidentity's `WorkloadIdentityCredential` holds assertion file contents for 10 minutes, which outlives a 5-minute token, so a pod can keep presenting an expired assertion even after the file is refreshed. Radius therefore builds its Azure workload identity credential from `ClientAssertionCredential` with a callback that reads the mounted file on every exchange. The .NET Bicep deployment engine and the Terraform `azurerm` provider keep their own readers, so file rotation is what covers them. If authentication still fails with an expired mounted JWT, the Go UCP credential reports the assertion's expiration instead of returning only the SDK's empty client-assertion detail.
...
**Custom resource types and recipe packs.** A repo that defines its own custom resource types places two files in the app's `.radius/` folder (next to `.radius/app.bicep`): a `custom-types.yaml` resource-type manifest and a `custom-recipe-pack.bicep` recipe pack. After the default provider pack and environment are created, the shared `apply-custom-recipe-packs` composite action registers the types with `rad resource-type create --from-file custom-types.yaml` (so the recipe pack's referenced types exist), compiles `custom-recipe-pack.bicep` to identify the literal names of non-existing `Radius.Core/recipePacks` resources authored by the template, deploys the file, and resolves those exact packs in the current workspace scope. Existing recipe-pack resources in the template are references rather than authored packs and are excluded from attachment. The action then reads the environment's current `recipePacks` with `rad env show --preview` and runs `rad env update <env> --recipe-packs <existing ∪ authored> --preview`. Because `--recipe-packs` replaces the list, the action preserves every pack already attached to the environment and adds only the packs authored by the repo. Deterministic identity discovery works both when a pack is created for the first time and when an idempotent redeploy updates a pack restored from prior state; it deliberately does **not** attach every pack from `rad recipe-pack list`, which spans all scopes and could pull unrelated packs into the environment and trip recipe-pack conflict validation. Dynamic pack-name expressions and conditional recipe-pack resources are rejected rather than guessed. Each file is optional and independent: an absent `custom-types.yaml` skips registration, an absent `custom-recipe-pack.bicep` keeps the default pack. The recipe pack is deployed from `.radius/` for the same `bicepconfig.json` resolution reason as `radius-env.bicep`.
...
The provider delete workflows run the same provider setup as deploy — OIDC login, cluster connection, cloud OIDC token projection, `rad credential register` — because `rad app delete` runs the resources' recipe delete path (e.g. `terraform destroy`) against the target cluster and cloud. The Azure delete action rotates the projected GitHub assertion for the lifetime of that operation, just like the deploy command action. They skip the deploy-only stages (create environment, deploy recipe pack, register registry credentials): the environment and its recipes come back from restored state, and deleting builds no images. The order is therefore restore state → delete → persist state. Restoring first (`rad startup`) is what makes the delete see the environment, recipe packs, resources, and Terraform state; persisting after (`rad shutdown`, run with `if: always()` in `teardown`) is what satisfies the requirement that the post-delete state is stored again, so the next operation plans against it. Both use the OCI state archive configured by `RADIUS_STATE_REGISTRY` and registry credentials; Git archival is no longer available.
...
- **Refresh the Azure federated token only once before an operation.** Rejected: the Azure SDK rereads its assertion file after its 10-minute cache expires, and a one-shot Secret still contains the original expired GitHub JWT. The command and delete actions rotate the mounted token for the lifetime of each operation instead.
- **Rotate the mounted assertion but keep `WorkloadIdentityCredential` in Radius.** Rejected: that credential caches file contents for 10 minutes, so a Go consumer can present a 5-minute GitHub assertion for another 5 minutes after it expires, no matter how often the file is rewritten. `ClientAssertionCredential` with a per-exchange file read removes the cache window entirely.
