...
- **Why**: the repository's shell instructions ask for four-space indentation, and a test that sits next to its subject should read the same way. The `.editorconfig` shfmt profile (`-bn -sr -kp`) was not applied: no script in the repository conforms to it, nothing enforces it, and `-kp` misaligns continuation lines. Matching the sibling script and the newest scripts in the tree is the useful notion of consistency here.
