...
- For downstream `workflow_dispatch`, prefer the native run ID returned by GitHub REST API version `2026-03-10`. A source-bound GitHub deployment receipt records the task and exact remote run URL; receipts share one `release-coordination` environment, are keyed by version and task, and are bound to the source SHA, so releases create no environments of their own. This avoids changing docs and samples run names or correlating runs by time windows. An upmerge pull request has no native identifier: it is bound once from the run's pull-request step and stored on the receipt. Write dispatch intent before calling the non-retried dispatch endpoint; if the response is lost before its ID is recorded, stop for operator reconciliation. Resume monitors or reruns that same run once its identity is known.
...
An upmerge workflow opening a PR is not completion: the receipt binds that pull request and becomes successful only when it has merged. docs and samples squash-merge, so reachability of the source commits from `edge` is never observable; a run with nothing to merge is complete. Sample tests start only after both upmerges complete. Both final preparation and final publication verify the receipts of the exact previous RC. The existing docs and samples publication workflows cut new channel branches, so finals invoke them after publication; patches run sample tests without recutting those branches. The release App token for this coordination has only Actions write, Contents read, and Pull requests read, with implicit Metadata read, on docs and samples. Radius deployment receipts and tag-build reruns use the built-in token.

The publication manifest reuses the parity observations and immutable digest locks. It checks actual downloaded binaries with native Go metadata inspection and installs the staged binary with the downloaded chart on an isolated kind cluster. Digest overrides isolate that installation from aliases that have not yet advanced. Finals and patches wait for environment approval after verification, then recheck every recorded output before mutation; RCs use the same mandatory gate without an approval wait. Stage summaries and optional Teams Adaptive Card notifications share the manifest and downstream receipts; a notification failure cannot change publication eligibility.
