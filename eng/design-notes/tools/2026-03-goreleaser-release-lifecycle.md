...
- For downstream `workflow_dispatch`, prefer the native run ID returned by GitHub REST API version `2026-03-10`. A source-bound GitHub deployment receipt records the task and exact remote run URL. This avoids changing docs and samples run names or correlating by time windows. Write dispatch intent before calling the non-retried dispatch endpoint; if the response is lost before its ID is recorded, stop for operator reconciliation. Resume monitors or reruns that same run once its identity is known.
...
An upmerge workflow opening a PR is not completion: its source must be reachable from `edge` before its receipt becomes successful. Sample tests start only after both upmerges complete. Both final preparation and final publication verify the receipts of the exact previous RC. The existing docs and samples publication workflows cut new channel branches, so finals invoke them after publication; patches run sample tests without recutting those branches. The release App token for this coordination has only Actions write and implicit Metadata read on docs and samples. Radius deployment receipts and tag-build reruns use the built-in token.
