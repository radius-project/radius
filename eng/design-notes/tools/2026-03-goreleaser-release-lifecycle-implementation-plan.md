...
| Signed Deployment Engine tag verification | `azure-octo/deployment-engine` | Publisher GitHub App (`contents: read`; the installation is extended to this repository in PR 12) | Cross-organization read of a private repository; `GITHUB_TOKEN` and the release App receive a 404 that is indistinguishable from a missing tag |
