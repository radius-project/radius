...
[korthout/backport-action](https://github.com/korthout/backport-action) was evaluated as a replacement for the backport scripts and rejected. It cherry-picks with `-x` and resolves the merge method automatically, which matches this design, but it commits locally and pushes with git, so backport commits could only be signed by introducing a bot GPG key - the same key-custody problem this plan avoids for Deployment Engine tags. It also opens every labelled backport simultaneously against the current branch head rather than serialising one per channel, and its `draft_commit_conflicts` mode commits conflict markers, which the completion check deliberately rejects. Adopting it would replace roughly the cherry-pick mechanics alone while giving up API-signed commits, approved-base pinning, and fail-closed conflict handling; the selection and completion-verification logic that most of the custom code implements has no equivalent in the action. Revisit if `release/*` gains a strict up-to-date merge requirement, which is what the one-open-backport-per-channel rule currently compensates for.
