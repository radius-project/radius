...
Add a PR-title validation job (pinned action such as `amannn/action-semantic-pull-request`) in non-blocking mode that annotates non-conforming titles with a fix-it comment. Define the allowed types to exactly match the git-cliff grouping introduced in PR 4 (proposed: `feat`, `fix`, `perf`, `refactor`, `docs`, `test`, `build`, `ci`, `chore`, `deps`, `revert`; scope optional; `!` allowed). Update the contributing documentation with the type table, examples, and the policy statement that types affect the changelog, never the version. Verify the repository squash-commit message setting is "pull request title" so merged commits actually carry the convention; change it if not.
