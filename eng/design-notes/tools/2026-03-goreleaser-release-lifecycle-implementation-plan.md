...
The built-in `GITHUB_TOKEN` cannot push to another repository or cross the organization boundary into `azure-octo`. Its tag pushes and PR creation do not trigger downstream workflows; explicit `workflow_dispatch` and `repository_dispatch` calls are exceptions to event suppression. Cross-repository writes and tag or PR creation that must trigger CI use a GitHub App installation token, minted per job with `actions/create-github-app-token`, scoped to the narrowest repository list and permission set the job needs. Same-repository reads, deployment receipts, and explicit dispatch or rerun APIs use `GITHUB_TOKEN`. No long-lived personal access tokens exist anywhere in the release path.
...
| Docs and samples workflow dispatch and monitoring                             | `docs`, `samples`                         | Release GitHub App (`actions: write`, `contents: read`, `pull_requests: read`, `metadata: read`) | Cross-repository `workflow_dispatch`, plus reading the channel branches and upmerge pull requests that prove completion; requires extending the App installation to both repositories, done in PR 16 |
| Coordination receipts and exact tag-build recovery                            | `radius`                                  | `GITHUB_TOKEN` (`deployments: write` for receipts, `actions: write` for reruns)                  | Durable native API state and same-repository recovery do not require another App scope                                                                                                               |
...
| Signed Deployment Engine tag verification                                     | `azure-octo/deployment-engine`            | Publisher GitHub App (`contents: read`, installation extended in PR 12)                          | Cross-organization read of a private repository; `GITHUB_TOKEN` and the release App receive a 404 that is indistinguishable from a missing tag                                                       |
...
- **App extension**: the publisher App installation on `azure-octo` gains `deployment-engine` with Contents read, so `Prepare Release` can verify the signed Deployment Engine tag before it changes anything; `GITHUB_TOKEN` and the release App cannot read that private repository, and a token without access would report an existing tag as missing.
...
Complete the design's verify and finalize stages: generate the release manifest, verify every asset name, checksum, image digest and platform set, Helm chart version and image references, external outputs, and binary metadata against the plan; run installation verification with the staged local `rad` binary before publication; move channel-alias promotion and draft publication behind this gate with the release-environment approval for finals and patches. The mandatory output set is identical for every release type; RCs publish automatically once the gate passes, per the decided gate policy. Dispatch and correlate docs and samples workflows (RC upmerge and sample tests as final-release gates; publication workflows post-release). Post stage transitions and the final summary to the Teams release channel through an incoming webhook, replacing manual transcription. Dispatching the docs and samples workflows and verifying their results requires extending the release App installation to those two repositories with `actions: write`, `contents: read`, and `pull_requests: read` (`metadata: read` is implicit); this is the only App-scope extension in the plan and is recorded in the token policy table.

Implementation reuses the parity collector, native `go version -m -json`, OCI inspectors, the existing SPDX validator, and the installation script. Installation uses the staged binary and chart with digest-pinned overrides, without changing the chart's published defaults ahead of PR 17. Finalization rechecks the stable observation snapshot after approval and persists the source-bound manifest before promoting aliases. PR 15's reconciliation-time environment approval is removed in this phase; final and patch approval belongs after verification, while RCs proceed automatically.

For docs and samples, the GitHub REST API version `2026-03-10` returns `workflow_run_id` directly from `workflow_dispatch`. Record that ID in native GitHub deployment statuses keyed by version, Radius source SHA, and task in one `release-coordination` environment, instead of changing the downstream run names or maintaining another discovery algorithm. Rerun failed jobs in the recorded run; an uncertain dispatch without a recorded ID stops for operator reconciliation rather than dispatching again. RC upmerge receipts bind the pull request each run opened and become successful only when that pull request has merged, because docs and samples squash-merge and the source commits never become reachable from `edge`; a run with nothing to merge completes without a pull request. Sample tests run afterward. Final preparation and publication require all three successful RC receipts. Finals dispatch the existing docs and samples release workflows; patches test the published version without recutting their channel branches.
...
The current dashboard and Deployment Engine publishers expose stable channel tags. Before publishing the chart, add full-version references to those verified digests with native ORAS tagging, without rebuilding or moving channel aliases. Verify the dashboard source commit against the plan and the Deployment Engine source and digest against its controller lock. The Radius workflow needs Actions package-write access to both GHCR packages; this uses `GITHUB_TOKEN`, not an expanded App identity. Staged verification inspects the full-version references while the historical parity collector and checked-in baselines retain the pre-migration contract. Chart upgrades use Helm's existing value-merging behavior: defaults advance to the chart version, explicit user overrides remain, and clearing an override adopts the new defaults.

- **Publisher prerequisite**: `radius-project/dashboard` must stamp `org.opencontainers.image.revision` with its built commit SHA. Its current build does not emit this label; a companion publisher change and a source-labeled release image are required before this draft can roll out. Do not waive the source check to adopt an unproven channel image.
- **Cleanup**: delete the helper's stable-version truncation and update chart verification expectations in the same PR. Keep channel publication and historical parity baselines unchanged.
