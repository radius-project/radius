...
**Narrative:** A developer asks Copilot for help deploying their application. Copilot confirms with the developer to enable Lattice for the repository. Once confirmed, Copilot analyzes the repository and produces an application graph. The developer reviews the graph on the Deployment panel, clicking through resources to see links back to source code. To continue with the deployment, Copilot tells the developer they must set up an environment first. Because the application has already been modeled, Copilot knows the application's infrastructure dependencies and can guide the developer through cloud provider details, authorization, and selecting a container platform and registry based on what the application requires. The developer provides any secrets or environment-specific configuration values needed at deployment time. Before deploying, the developer previews what cloud resources will be provisioned, then confirms the deployment. They can track progress on the Deployment panel and are notified when the deployment completes or fails. After deployment, the developer can inspect cloud resource properties, navigate to cloud console pages, and view deployment history. In the future, the developer will also be able to access the app via a public URL or custom domain, view application logs from deployed cloud resources, and show or hide the Deployment panel via a Copilot menu item.
...
| - | --- | ---------- | --------- | ----- |
| 1 | p0 | As a developer, when I ask Copilot to deploy my application, Copilot asks if I want to enable Lattice for my repository so that deployment capabilities are available for my project. | Q10 | TBD |
| 2 | p0 | As a developer, after I have enabled Lattice, Copilot will analyze my repository and produce an application graph from my source code, Dockerfiles, and packages. | Q6, Q23 | TBD |
| 3 | p0 | As a developer, I can view the application graph on the Deployment panel showing my application's resources, properties of each application resource, and their relationships. | Q6, Q7 | TBD |
| 4 | p0 | As a developer, I can click on any of my application's resources to get a link to the source code that requires the resource (e.g., a container image has a link to a Dockerfile). | Q7, Q23 | TBD |
| 5 | p0 | As a developer, after the application graph has been created, I am prompted to create an AWS or Azure environment. | Q6, Q12 | TBD |
| 6 | p0 | As a developer, when Copilot is creating an environment, Copilot asks me details about my cloud environment such as the AWS account ID and region, or Azure subscription ID and resource group. | Q12 | TBD |
| 7 | p0 | As a developer, when Copilot is creating an environment, Copilot guides me through authorizing GitHub to manage resources within my cloud provider. | Q13, Q23 | TBD |
| 8 | p0 | As a developer, when Copilot is creating an environment, Copilot helps me select a container platform, an OCI registry, and other infrastructure dependencies required by my application. | Q6, Q12 | TBD |
| 9 | p1 | As a developer, I can provide secrets and environment-specific configuration values (e.g., database passwords, API keys) that are stored securely and made available to my application at deployment time. | -- | TBD |
| 10 | p1 | As a developer, before my application is deployed to my environment I can preview what cloud resources will be provisioned and their properties. | Q6, Q23 | TBD |
| 11 | p0 | As a developer, I can ask Copilot to deploy my application to one of my environments. I can see deployment progress and status on the Deployment panel. | Q6, Q14 | TBD |
| 12 | p1 | As a developer, I am notified when a deployment completes or fails, even if I have navigated away from the Deployment panel. | -- | TBD |
| 13 | p1 | As a developer, when a deployment fails, I can see the error details on the Deployment panel and understand what went wrong so I can fix and retry. | -- | TBD |
| 14 | p0 | As a developer, I can click a cloud resource and see its properties (e.g., the ARN for an AWS RDS database, a SKU for an Azure database, or a URL for a Kubernetes Service). | Q7, Q23 | TBD |
| 15 | p1 | As a developer, I can click a cloud resource on the application graph to navigate to its cloud console page. | Q7, Q23 | TBD |
| 16 | p1 | As a developer, I can view the history of deployments to an environment, including what was deployed, when, and whether it succeeded or failed. | -- | TBD |
| 17 | p3 | As a developer, I can easily show and hide the Deployment panel via a Copilot menu item. | Q7 | TBD |
| 18 | p2 | As a developer, after deploying my application, I can access it via a public URL or custom domain. | -- | TBD |
| 19 | p2 | As a developer, I can view application logs from my deployed cloud resources through the Copilot app. | -- | TBD |
...
| # | Pri | User Story | PRFAQ Ref | Owner |
| - | --- | ---------- | --------- | ----- |
| 20 | p1 | As a developer, I can ask Copilot to modify my application graph (add, remove, or change resources and their properties) and Copilot updates the graph. | Q11 | TBD |
| 21 | p1 | As a developer, when I ask Copilot to modify my application code (e.g., adding a database), Copilot adds the corresponding resource to the application graph and adds the appropriate database client library in the application source code. Copilot keeps the application code and the application graph in sync. | Q11, Q23 | TBD |
| 22 | p2 | As a developer, I can click on any of my application's resources on the Deployment panel and edit the properties. | Q7 | TBD |
| 23 | p0 | As a developer, after I have made a change to my application graph, I can ask Copilot to update a deployment in one of my environments. I'm presented with a diff of what will change before I approve the deployment. | Q6, Q14 | TBD |
| 24 | p2 | As a developer, I can ask Copilot whether my deployed application has drifted from the application graph. I am shown which resources and which resource properties are different. | Q15, Q24 | TBD |
| 25 | p2 | As a developer, after asking Copilot whether my deployed application has drifted, I can tell Copilot to redeploy to match the application graph. | Q15, Q24 | TBD |
| 26 | p3 | As a developer, after asking Copilot whether my deployed application has drifted, I can tell Copilot to update the graph to match reality. | Q15, Q24 | TBD |
...
| # | Pri | User Story | PRFAQ Ref | Owner |
| - | --- | ---------- | --------- | ----- |
| 27 | p0 | As a developer, I can ask Copilot to create additional named environments. I am taken through the same environment creation flow. | Q12 | TBD |
| 28 | p1 | As a developer, I can view my existing environments and their cloud provider details, including which applications are deployed, when they were last updated, and whether they have deployment errors. | -- | TBD |
| 29 | p1 | As a developer, I can ask Copilot to promote my deployed application from one environment to another, review the diff of what will change, and approve the deployment. | Q16 | TBD |
| 30 | p1 | As a developer, I can roll back a deployment to a previous version if a new deployment breaks my application. | -- | TBD |
...
| # | Pri | User Story | PRFAQ Ref | Owner |
| - | --- | ---------- | --------- | ----- |
| 31 | p1 | As a developer on a team that uses pull requests, after I make a change to the application graph, I need my coworker to approve the change before I can deploy to one of my environments. | Q7 | TBD |
| 32 | p1 | As a PR approver, when I'm reviewing a PR, I expect to be presented with a diff of what changed to the application graph. | Q7 | TBD |
| 33 | p2 | As a PR approver, when I'm reviewing a PR, I can ask Copilot to preview how the deployed cloud resources will change in a specific environment once I approve the PR. | Q7, Q23 | TBD |
...
| 35 | p1 | As a developer, I can ask Copilot to delete an application entirely, which deletes the application from all of its environments and removes the application graph from my repository. | Q17 | TBD |
...
| # | Pri | User Story | PRFAQ Ref | Owner |
| - | --- | ---------- | --------- | ----- |
| 38 | p2 | As a developer, I can define and deploy multiple independent applications from a single repository (e.g., a monorepo containing several applications that are deployed separately). | -- | TBD |
| 39 | p2 | As a developer, I can deploy applications that span multiple repositories (e.g., a frontend repo and a backend repo) as a single application. | -- | TBD |
...
| 43 | p3 | As a developer, I can declaratively create multiple environments. | -- | TBD |
| 44 | p3 | As a developer, I can deploy my application to my local development workstation for testing before deploying to the cloud. | Q9 | TBD |
